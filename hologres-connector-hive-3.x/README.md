...
* 永久：
    * 将hologres-connector-hive-3.x-1.5.6-SNAPSHOT-jar-with-dependencies.jar放在HiveServer3所在节点的$HIVE_HOME/auxlib目录下（目录不存在就新建）
* session级
    1. 将hologres-connector-hive-3.x-1.5.6-SNAPSHOT-jar-with-dependencies.jar上传至hdfs
...
| akv4_enabled|  false   | 否| 默认值为false, 表示不启用akv4认证  | 
| akv4_region|  无   | 否| 无默认值, 在启用akv4认证时,需要根据实例所在的region设置,例如cn-beijing  |
