...
/*
 * Shallow copy a doubly linked list.
 * Previous state will be thrown away without any cleanup.
 */
static inline void
dlist_copy(dlist_head *dst, dlist_head* src)
{
	*dst = *src;
...
/*
 * Iterate through the list pointed at by 'from' storing the state in 'iter'.
 *
 * Access the current element with iter.cur.
 *
 * It is *not* allowed to manipulate the list during iteration.
 */
#define dlist_foreach_from(iter, lhead, from)								\
	for ((iter).end = &(lhead)->head,										\
		 (iter).cur = (from);											\
		 (iter).cur != (iter).end;											\
		 (iter).cur = (iter).cur->next)
