...
void metrics_gather_and_show(Metrics* metrics){
    ...
    LOG_INFO("---------------------- Metrics of last %ldms ----------------------", metrics->reportInterval);
    if (endTime - metrics->startTime != 0)              LOG_INFO("           Queries - Tot: %-7lld Cnt: %-7lld QPS: %lld", metricsGather->qps->value, metricsGather->qps->count, metricsGather->qps->value*1000/(endTime - metrics->startTime));
    if (endTime - metrics->startTime != 0)              LOG_INFO("           Records - Tot: %-7lld Cnt: %-7lld RPS: %lld", metricsGather->rps->value, metricsGather->rps->count, metricsGather->rps->value*1000/(endTime - metrics->startTime));
    if (metrics->timeoutFlush->count != 0)              LOG_INFO("   Timeout Flushes - Tot: %-7lld Cnt: %-7lld Pct: %.1lf%%", metrics->timeoutFlush->value, metrics->timeoutFlush->count, (double)metrics->timeoutFlush->value/metrics->timeoutFlush->count * 100);
    if (metrics->actionSize->count != 0)                LOG_INFO("Requests in Action - Cnt: %-7lld Avr: %-7lld Min: %-7ld Max: %ld", metrics->actionSize->count, metrics->actionSize->value/metrics->actionSize->count, metrics->actionSize->min, metrics->actionSize->max);
    if (metricsGather->execPreparedTime->count != 0)    LOG_INFO("Exec Prepared Time - Cnt: %-7lld Avr: %-7lld Min: %-7ld Max: %ld", metricsGather->execPreparedTime->count, metricsGather->execPreparedTime->value/metricsGather->execPreparedTime->count, metricsGather->execPreparedTime->min, metricsGather->execPreparedTime->max);
    if (metricsGather->handleActionTime->count != 0)    LOG_INFO("Handle Action Time - Cnt: %-7lld Avr: %-7lld Min: %-7ld Max: %ld", metricsGather->handleActionTime->count, metricsGather->handleActionTime->value/metricsGather->handleActionTime->count, metricsGather->handleActionTime->min, metricsGather->handleActionTime->max);
    if (metricsGather->idleTime->count != 0)            LOG_INFO("  Worker Idle Time - Cnt: %-7lld Avr: %-7lld Min: %-7ld Max: %ld", metricsGather->idleTime->count, metricsGather->idleTime->value/metricsGather->idleTime->count, metricsGather->idleTime->min, metricsGather->idleTime->max);
    // if (metrics->gatherTime->count != 0)                LOG_INFO("       Gather time - Cnt: %lld  Avr: %lld, Min: %ld, Max: %ld", metrics->gatherTime->count, metrics->gatherTime->value/metrics->gatherTime->count, metrics->gatherTime->min, metrics->gatherTime->max);
    if (metrics->getFutureTime->count != 0)             LOG_INFO("   Get Future Time - Cnt: %-7lld Avr: %-7lld Min: %-7ld Max: %ld", metrics->getFutureTime->count, metrics->getFutureTime->value/metrics->getFutureTime->count, metrics->getFutureTime->min, metrics->getFutureTime->max);
    if (metrics->submitActionTime->count != 0)          LOG_INFO("Submit Action Time - Cnt: %-7lld Avr: %-7lld Min: %-7ld Max: %ld", metrics->submitActionTime->count, metrics->submitActionTime->value/metrics->submitActionTime->count, metrics->submitActionTime->min, metrics->submitActionTime->max);
