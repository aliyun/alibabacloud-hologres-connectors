...
public class HologresSinkTableITTest extends HologresTestBase {
    ...
    public static List<String> parameters() {
        return Arrays.asList("insert", "insert_fixedfe", "stream", "bulkload");
    }
    ...
    public HologresSinkTableITTest(String sdkMode) throws IOException {
        ...
        switch (sdkMode) {
            ...
            case "insert_fixedfe":
                ...
                break;
            case "stream":
                this.writeMode = WriteMode.COPY_STREAM;
                break;
            case "bulkload":
                this.writeMode = WriteMode.COPY_BULK_LOAD_ON_CONFLICT;
                break;
        }
    }
        ...
        tEnv.executeSql(
                String.format(
                        "CREATE TABLE sinkTable ("
                                ...
                                + "'connection.fixed'='%s',"
                                + "'sink.write-mode'='%s',"
                                + "'sink.on-conflict-action'='insertorignore',"
                                + "'sink.aggressive.enabled'='true',"
                                + "'endpoint'='%s',"
                                + "'dbName'='%s',"
                                + "'tableName'='%s',"
                                + "'userName'='%s',"
                                + "'password'='%s'"
                                + ")",
                        fixedMode, writeMode, endpoint, database, sinkTable, username, password));
    ...
    public void testSinkTableUpdate() throws Exception {
        if (writeMode == WriteMode.COPY_BULK_LOAD) {
            writeMode = WriteMode.COPY_BULK_LOAD_ON_CONFLICT;
        }
        Object[][] prepare =
                new Object[][] {
                    new Object[] {0, "old_1"},
        ...
        tEnv.executeSql(
                String.format(
                        "CREATE TABLE sinkTable ("
                                ...
                                + "'sink.write-mode'='%s',"
                                + "'sink.on-conflict-action'='insertorignore',"
                                + "'sink.copy.format'='text',"
                                + "'connection.pool.name'='pool',"
                                + "'endpoint'='%s',"
                                + "'dbName'='%s',"
                                + "'tableName'='%s',"
                                + "'userName'='%s',"
        ...
        tEnv.executeSql(
                String.format(
                        "create table sinkTable"
                                ...
                                + "'connector'='hologres',"
                                + "'connector'='hologres',"
                                + "'sink.check-and-put.column'='C,C',"
                                + "'sink.check-and-put.null-as'='2023-10-10 12:00:00',"
                                + "'sink.check-and-put.operator'='GREATER_OR_EQUAL',"
    ...
    @Test
    public void testSinkTablePutRecordWithExpression() throws Exception {
        ...
        checkResultWithTimeout(
                new String[] {
                    "5,1,dim0,null",
                    "3,1,dim0,null",
                    "7,1,dim0dim2,2023-10-10 12:00:00.0",
                    "3,1,dim0dim2,2023-10-10 12:00:00.0",
                    "7,1,dim0dim2dim3,2023-10-10 14:00:00.0",
                    "3,1,dim0dim2dim3,2023-10-10 14:00:00.0",
                    "7,1,dim0dim2dim3dim5,2023-10-10 14:00:00.0",
                    "3,1,dim0dim2dim3dim5,2023-10-10 14:00:00.0",
                    "7,1,dim0dim2dim3dim5dim6,2023-10-10 15:00:00.0"
                },
                "select hg_binlog_event_type,* from " + sinkTableName,
                new String[] {"hg_binlog_event_type", "a", "b", "C,C"},
                JDBCUtils.getDbUrl(endpoint, database),
