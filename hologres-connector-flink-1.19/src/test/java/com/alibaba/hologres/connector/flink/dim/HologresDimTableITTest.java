package com.alibaba.hologres.connector.flink.dim;
...
public class HologresDimTableITTest extends HologresTestBase {
    ...
    Object[] expected =
            new Object[] {
                Row.of(
                        ...
                        "{\"a\":\"bbbb\", \"c\":\"dddd\"}",
                        "{\"a\": \"bbbb\", \"c\": \"dddd\"}",
                        LocalTime.of(18, 17, 36) // "18:17:36.789"
                        ),
                Row.of(
                        "Hi2",
                        652482L,
    ...
    public void testDimTableWithPartitionTable() {
        ...
        try {
            TableResult result =
                    tEnv.executeSql(
                            String.format(
                                    "SELECT T.ip, a, b FROM MyTable AS T JOIN dim /*+ OPTIONS('connection.fixed'='%s') */"
    ...
    private void runTest(String tableName) {
        TableResult result =
                tEnv.executeSql(
                        String.format(
                                "SELECT T.ip, e, f, g, h, j, n, o, p, r, a, b, d, u, v, w FROM default_catalog.default_database.MyTable AS T JOIN %s /*+ OPTIONS('connection.fixed'='%s') */"
                                        + " FOR SYSTEM_TIME AS OF T.proctime "
                                        + "AS H ON T.name = H.a",
    ...
    private void runOne2ManyTest() {
        try {
            TableResult result =
                    tEnv.executeSql(
                            String.format(
                                    "SELECT T.ip, a, b, c FROM MyTable AS T JOIN dim /*+ OPTIONS('connection.fixed'='%s') */"
                                            + " FOR SYSTEM_TIME AS OF T.proctime "
                                            + "AS H ON T.name = H.a",
                                    fixedConnectionMode));
                    ...
                    tEnv.executeSql(
                            String.format(
                                    "SELECT H.user_name, H.user_id FROM default_catalog.default_database.MyTable AS T JOIN "
                                            + " dim /*+ OPTIONS('lookup.insert-if-not-exists'='true', 'connection.fixed'='%s')*/ FOR SYSTEM_TIME AS OF T.proctime "
