...
public class ExecutionPoolTest extends HoloClientTestBase {
    ...
    public void testExecutionPool001() throws Exception {
        ...
        try (Connection conn = buildConnection();
                ExecutionPool pool = ExecutionPool.buildOrGet("executionPool001", config, false)) {
    ...
    /** ExecutionPool Method: put(Put put). different writeMode use one ExecutionPool */
    @Test
    public void testExecutionPool002() throws Exception {
        ...
        config0.setOnConflictAction(OnConflictAction.INSERT_OR_UPDATE);
        ...
        try (Connection conn = buildConnection()) {
            ...
            try (HoloClient client0 = new HoloClient(config0);
                    HoloClient client1 = new HoloClient(config1)) {
                ExecutionPool pool =
                        ExecutionPool.buildOrGet("testExecutionPool002", config0, false);
                ...
                try (Statement stat = conn.createStatement()) {
                    try (ResultSet rs =
                            stat.executeQuery("select count(hg_binlog_lsn) from " + tableName0)) {
                        if (rs.next()) {
                    ...
                    try (ResultSet rs =
                            stat.executeQuery("select count(hg_binlog_lsn) from " + tableName1)) {
                        if (rs.next()) {
                            // 1 insert
    ...
    public void testExecutionPool003() throws Exception {
        ...
        fixedConfig.setOnConflictAction(OnConflictAction.INSERT_OR_UPDATE);
        ...
        try (Connection conn = buildConnection();
                ExecutionPool pool =
                        ExecutionPool.buildOrGet(
                                "executionPool003", config, false, config.isUseFixedFe());
                ExecutionPool fixedPool =
                        ExecutionPool.buildOrGet(
                                "fixedExecutionPool003",
                                fixedConfig,
                                false,
                                fixedConfig.isUseFixedFe())) {
    ...
    /** ExecutionPool Method: setFixedPool. */
    @Test
    public void testExecutionPool004() throws Exception {
        ...
        // set FixedPool only
        try (ExecutionPool fixedPool =
                ExecutionPool.buildOrGet(
                        "fixedExecutionPool004", fixedConfig, false, fixedConfig.isUseFixedFe())) {
        ...
        try (ExecutionPool pool =
                ExecutionPool.buildOrGet(
                        "ExecutionPool004", config, false, config.isUseFixedFe())) {
            Class<HoloClient> clientClass = HoloClient.class;
            Class<ActionCollector> collectorClass = ActionCollector.class;
            HoloClient client0 = new HoloClient(config);
            client0.setPool(pool);
    ...
    /**
     * ExecutionPool Method: put(Put put). different writeMode use one ExecutionPool, use fixed fe
     */
    @Test
    public void testExecutionPool005() throws Exception {
        ...
        try (Connection conn = buildConnection()) {
            ...
            try (HoloClient client0 = new HoloClient(config0);
                    HoloClient client1 = new HoloClient(config1)) {
                ExecutionPool pool =
                        ExecutionPool.buildOrGet("testExecutionPool005", config0, false);
                ExecutionPool fixedPool =
                        ExecutionPool.buildOrGet(
                                "fixed-testExecutionPool005", config0, false, true);
                ...
                fixedPool =
                        ExecutionPool.buildOrGet(
                                "fixed-testExecutionPool005", config1, false, true);
    ...
    /**
     * ExecutionPool Method: put(Put put). buildOrGet use same name but different config, one use
     * fixed fe, the other not
     */
    @Test
    public void testExecutionPool006() throws Exception {
        ...
        try (Connection conn = buildConnection()) {
            ...
            try (HoloClient client0 = new HoloClient(config0);
                    HoloClient client1 = new HoloClient(config1)) {
                ExecutionPool pool0 =
                        ExecutionPool.buildOrGet("testExecutionPool006", config0, false);
                ...
                ExecutionPool pool1 =
                        ExecutionPool.buildOrGet("testExecutionPool006", config1, false);
    ...
    /** ExecutionPool Method: put(Put put). 复用pool多线程测试，主要用于验证是否存在可能的死锁. 测试时间比较久，默认不跑. */
    @Ignore
    @Test
    public void testExecutionPool007() throws Exception {
        ...
        for (int c = 0; c < 100; c++) {
            ...
            try (ExecutionPool pool =
                    ExecutionPool.buildOrGet("testExecutionPool007", config, false)) {
                ...
                Runnable insert =
                        () -> {
                            ...
                            String tableName =
                                    "holO_client_execution_pool_007_" + id.getAndIncrement();
                            try (HoloClient client = new HoloClient(config1)) {
                                ExecutionPool pool0 =
                                        ExecutionPool.buildOrGet(
                                                "testExecutionPool007", config1, false);
