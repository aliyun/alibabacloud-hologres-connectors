...
/** Binlog Decoder 测试. holo实例需要大于1.1.2版本. 测试数据库需已经开启create extension roaringbitmap. */
public class BinlogReaderTest extends HoloClientTestBase {
    ...
    /** binlogGroupShardReader. binlogDecoder. */
    @Test
    public void binlogReader001() throws Exception {
        if (properties == null) {
            return;
        }
        ...
        try (Connection conn = buildConnection();
            ...
            String tableName = "holo_client_binlog_decoder_001";
            ...
            try {
                ...
                try (Connection conne2 =
                        DriverManager.getConnection(properties.getProperty("url"), info)) {
    ...
    /** binlogGroupShardReader. binlogDecoder decompress. */
    @Test
    public void binlogReader002() throws Exception {
        if (properties == null) {
            return;
        }
        boolean supportCompression = holoVersion.compareTo(new HoloVersion(3, 2, 0)) >= 0;
        boolean[] testCompressions = new boolean[] {false, supportCompression};
        HoloConfig config = buildConfig();
        config.setOnConflictAction(OnConflictAction.INSERT_OR_REPLACE);
        try (Connection conn = buildConnection();
                HoloClient client = new HoloClient(config)) {
            ...
            try {
                ...
                for (boolean testCompression : testCompressions) {
                    try (Connection conne2 =
                            DriverManager.getConnection(properties.getProperty("url"), info)) {
                        ...
                        try (PgConnection pgConnection = conne2.unwrap(PgConnection.class)) {
                            ...
                            ChainedLogicalStreamBuilder builder =
                                    pgConnection
                                            ...
                                            .withSlotOption("table_name", tableName)
                                            .withSlotOption("parallel_index", 0)
                                            .withSlotOption("batch_size", 50);
                            if (testCompression) {
                                builder = builder.withSlotOption("enable_compression", true);
                            }
                            PGReplicationStream pgReplicationStream = builder.start();

                            HoloBinlogDecoder decoder =
                                    new HoloBinlogDecoder(
                                            schema, new String[0], testCompression, false, false);
                            ...
                            for (BinlogRecord record : records) {
                                ...
                                Assert.assertEquals(record.getObject("amount").toString(), "16.21");
                                if (id == 2) {
                                    Assert.assertNull(record.getObject("t"));
                                } else if (id == 3) {
                                    Assert.assertEquals(record.getObject("t"), "NULL");
                                } else {
                                    Assert.assertEquals(record.getObject("t"), "abc,d");
                                }
                                Assert.assertEquals(
                                        record.getObject("ts").toString(), "2021-04-12 12:12:12.0");
                                Assert.assertEquals(
                                        record.getObject("t_a"), new String[] {"a", "b,c"});
                                Assert.assertEquals(
                                        record.getObject("i_a"), new int[] {1, 2, 3, 4, 5});
                                Assert.assertEquals(
                                        record.getObject("ba"), new byte[] {(byte) (id % 128)});
                            }
                            Assert.assertEquals(50, records.size());
                            pgReplicationStream.forceUpdateStatus();
                            pgReplicationStream.close();

                            Tuple<Long, Long> tuple = decoder.trace();
                            if (testCompression) {
                                Assert.assertTrue(tuple.l < tuple.r);
                            } else {
                                Assert.assertEquals(0, tuple.r);
                                Assert.assertEquals(0, tuple.l);
                            }
                        }
                    }
                }
    ...
    public void binlogReader003() throws Exception {
        ...
        TimeZone[] timeZoneList =
                new TimeZone[] {
                    TimeZone.getTimeZone(ZoneOffset.ofHours(-5)),
                    TimeZone.getTimeZone(ZoneOffset.ofHours(-2)),
                    TimeZone.getTimeZone(ZoneOffset.ofHours(0)),
                    TimeZone.getTimeZone(ZoneOffset.ofHours(2)),
                    TimeZone.getTimeZone(ZoneOffset.ofHours(5)),
                    TimeZone.getTimeZone(ZoneOffset.ofHours(8)),
                    TimeZone.getTimeZone(ZoneOffset.ofHours(10)),
        ...
        try (Connection conn = buildConnection();
            ...
            try {
                ...
                for (TimeZone timeZone : timeZoneList) {
                    ...
                    String startTime = new Timestamp(currentTime).toLocalDateTime().toString();
                    LOG.info("Test TimeZone {} start at {}", timeZone.toZoneId(), startTime);
                    reader =
                            client.binlogSubscribe(
                                    Subscribe.newStartTimeBuilder(tableName)
                                            .setBinlogReadStartTime(startTime)
                                            .build());

                    long start = System.nanoTime();
                    int count = 0;
                    BinlogRecord r;
                    while ((r = reader.getBinlogRecord(60000)) != null) {
                        if ((int) r.getObject("id") < 500) {
                            Assert.fail("id < 500");
                        }
                        count++;
                        if (count == 500) {
                    ...
                    LOG.info(
                            "Binlog reader count: {}, cost: {} ms",
                            count,
                            (end - start) / 1000000L);

                    Assert.assertEquals(count, 500);
                    LOG.info("Test TimeZone {} success", timeZone.toZoneId());
                }
            } finally {
                if (reader != null) {
                    reader.cancel();
    ...
    /** binlogGroupShardReader. 读取1000条binlog时间 */
    @Test
    public void binlogReaderDefaultCreateSlot() throws Exception {
        if (properties == null || holoVersion.compareTo(needVersion) < 0) {
            return;
        }
        HoloConfig config = buildConfig();
        ...
        try (Connection conn = buildConnection();
                HoloClient client = new HoloClient(config)) {
            String schemaName = "\"Test 001\"";
            String tableName = "\"Test 001\".\"Holo_client_binlog_reader_create_slot\"";
            ...
            String createSql =
                    "create table "
                            ...
                            + "', 'shard_count', '1');\n";
            execute(conn, new String[] {dropSql});
            execute(conn, new String[] {createSchema});
            execute(conn, new String[] {"begin;", createSql, "commit;"});
            ...
            try {
                TableSchema schema = client.getTableSchema(tableName, true);
                String slotName = Command.getOrCreateDefaultSlot(client, schema.getTableNameObj());
                ...
                while ((r = reader.getBinlogRecord()) != null) {
                    int id = (Integer) r.getObject("id");
                    Assert.assertEquals(r.getObject("amount").toString(), "16.21");
                    if (id == 2) {
                        Assert.assertEquals(r.getObject("t"), null);
                    } else if (id == 3) {
                        Assert.assertEquals(r.getObject("t"), "NULL");
                    } else {
                        Assert.assertEquals(r.getObject("t"), "abc,d");
                    }
                    Assert.assertEquals(
                            r.getObject("ts"), Timestamp.valueOf("2021-04-12 12:12:12.0"));
                    Assert.assertEquals(r.getObject("ba"), new byte[] {(byte) (id % 128)});
                    Assert.assertEquals(r.getObject("t_a"), new String[] {"a", "b,c"});
                    Assert.assertEquals(r.getObject("i_a"), new int[] {1, 2, 3, 4, 5});
                    count++;
                    if (count == 1000) {
                        reader.cancel();
                        break;
    ...
    /** binlogGroupShardReader. 读取部分字段 */
    @Test
    public void binlogReaderProjectionColumns() throws Exception {
        HoloVersion needVersion = new HoloVersion(3, 2, 0);
        if (properties == null || holoVersion.compareTo(needVersion) < 0) {
            return;
        }
        HoloConfig config = buildConfig();
        ...
        try (Connection conn = buildConnection();
                HoloClient client = new HoloClient(config)) {
            String tableName = "holo_client_binlog_reader_projection_columns";
            ...
            try {
                ...
                for (int i = 0; i < 1000; ++i) {
                    ...
                    put2.setObject("ts", "2021-04-12 12:12:12");
                    put2.setObject("ba", "{\"a\":\"" + i + "\"}");
                    put2.setObject("t_a", new String[] {"a", "b,c"});
                    put2.setObject("i_a", new int[] {1, 2, 3, 4, i});
                    client.put(put2);
                }
                client.flush();
                            ...
                            client.binlogSubscribe(
                                    Subscribe.newStartTimeBuilder(tableName)
                                            ...
                                            .addProjectionColumnNamesToSubscribe(
                                                    new String[] {"id", "t", "baa", "i_a"})
                                            .build());
                } catch (HoloClientException e) {
                    if (!e.getMessage()
                            .contains(
                                    "projection column names [id, t, baa, i_a] is not subset of table columns")) {
                        throw e;
                    }
                }
                reader =
    ...
    /** binlogGroupShardReader. 设置consumerGroup, 查询stat activity */
    @Test
    public void binlogReaderSetConsumerGroup() throws Exception {
        HoloVersion needVersion = new HoloVersion(4, 0, 0);
        if (properties == null || holoVersion.compareTo(needVersion) < 0) {
            return;
        }
        HoloConfig config = buildConfig();
        ...
        boolean[] useFixedFe = new boolean[] {false, true};
        for (boolean useFixedFeItem : useFixedFe) {
            config.setUseFixedFe(useFixedFeItem);

            try (Connection conn = buildConnection();
                    HoloClient client = new HoloClient(config)) {
                String tableName = "holo_client_binlog_reader_consumer_group" + useFixedFeItem;
                String consumerGroup = "test_consumer_group_01";
                ...
                try {
                    ...
                    int count = 0;
                    BinlogRecord record;
                    Map<Integer, Long> shardId2Lsn = new HashMap<>();
                    while ((record = reader.getBinlogRecord()) != null) {
                        Assert.assertEquals(record.getBitSet().cardinality(), 4);
                        int id = (int) record.getObject("id");
                        if (id == 2) {
                        ...
                        Assert.assertEquals(record.getObject("i_a"), new int[] {1, 2, 3, 4, id});
                        count++;
                        shardId2Lsn.put(record.getShardId(), record.getBinlogLsn());
                        if (count == 1000) {
                            break;
                        }
                    }
                    ...
                    try (Statement statement = conn.createStatement()) {
                        try (ResultSet resultSet =
                                statement.executeQuery(
                                        "SELECT\n"
                                                + "    extend_info::json -> 'binlog_status' -> 'shard_id' AS binlog_status_shard_id,\n"
                                                + "    extend_info::json -> 'binlog_status' ->> 'table_name' AS binlog_status_table_name,\n"
                                                + "    extend_info::json -> 'binlog_status' ->> 'consumer_group' AS binlog_status_consumer_group,\n"
                                                + "    extend_info::json -> 'binlog_status' -> 'emit_lsn' AS binlog_status_emit_lsn,\n"
                                                + "    to_timestamp((extend_info::json -> 'binlog_status' -> 'emit_timestamp')::text::bigint / 1000000.0) AS binlog_status_emit_timestamp\n"
                                                + "FROM\n"
                                                + (useFixedFeItem
                                                        ? "hg_fixed_fe_stat_activity\n"
                                                        : "hg_stat_activity\n")
                                                + "WHERE\n"
                                                + "    extend_info::json -> 'binlog_status' ->> 'consumer_group' = '"
                                                + consumerGroup
                                                + "';")) {
                            Assert.assertTrue(resultSet.next());
                            while (resultSet.next()) {
                                int shardId = resultSet.getInt(1);
                                Long emitLsn = resultSet.getLong(4);
                                Assert.assertTrue(shardId2Lsn.get(shardId) >= emitLsn);
                                LOG.info(
                                        "shardId: {}, emitLsn: {}, lastLsn: {}",
                                        shardId,
                                        emitLsn,
                                        shardId2Lsn.get(shardId));
                            }
                        }
                    }
    ...
    /** binlogGroupShardReader. 带filter */
    @Test
    public void binlogReaderFilter() throws Exception {
        ...
        try (Connection conn = buildConnection();
                HoloClient client = new HoloClient(config)) {
            String tableName = "holo_client_binlog_reader_filter";
            ...
            try {
                        ...
                        client.binlogSubscribe(
                                Subscribe.newStartTimeBuilder(tableName)
                                        ...
                                        .addProjectionColumnNamesToSubscribe(
                                                new String[] {"i_a", "t", "id", "ba", "id"})
                                        .addBinlogFilter(
                                                schema.getColumn(schema.getColumnIndex("t")),
                ...
                // filter by binlog_event_type
                Put put3 = new Put(schema);
                put3.setObject("id", 2);
                put3.getRecord().setType(Put.MutationType.DELETE);
                client.put(put3);
                client.flush();
                Column binlog_event_type = new Column();
                binlog_event_type.setName("hg_binlog_event_type");
                binlog_event_type.setType(Types.BIGINT);
                ...
                while ((record = reader.getBinlogRecord()) != null) {
                    Assert.assertEquals(record.getBitSet().cardinality(), 4);
                    Assert.assertEquals(record.getObject("t"), null);
                    Assert.assertEquals(record.getBinlogEventType(), BinlogEventType.DELETE);
                    count++;
                    if (count == 1) {
                        reader.cancel();
                        break;
    ...
    /** binlogGroupShardReader. 指定start time */
    @Test
        ...
        try (Connection conn = buildConnection();
            ...
            try {
                ...
                try (Statement stat = conn.createStatement()) {
                    ResultSet rs =
                            stat.executeQuery(
                                    "select hg_shard_id, max(hg_binlog_lsn) from "
                                            + tableName
                        ...
                        client.binlogSubscribe(
                                Subscribe.newOffsetBuilder(tableName)
                                        .addShardsStartOffset(
                                                ...
                                                new BinlogOffset().setTimestamp(now.toString()))
                                        .build());
    ...
    /** binlogGroupShardReader. 读取100000条binlog时间 */
    @Test
    ...
    /** binlogGroupShardReader. 抛出超时异常 */
    @Test
    public void binlogReader006() throws Exception {
        if (properties == null || holoVersion.compareTo(needVersion) < 0) {
            return;
        }
        HoloConfig config = buildConfig();
        ...
        try (Connection conn = buildConnection();
                HoloClient client = new HoloClient(config)) {
            String tableName = "holo_client_binlog_reader_006";
            ...
            try {
                ...
                try {
                    int count = 0;
                    while (reader.getBinlogRecord(1) != null) {
                        count++;
                        if (count == 1000) {
                            reader.cancel();
                            break;
                    ...
                    Assert.assertTrue(
                            e.getMessage()
                                    .contains(
                                            "Binlog record fetch timed out. This may occur if the job has experienced backpressure"));
                }
            } finally {
                if (reader != null) {
                    reader.cancel();
    ...
    /** binlogGroupShardReader. update and binlogIgnoreBeforeUpdate=false */
    @Test
    ...
    /** binlogGroupShardReader. update and binlogIgnoreBeforeUpdate=true */
    @Test
    public void binlogReader011() throws Exception {
        ...
        try (Connection conn = buildConnection();
            ...
            try {
                ...
                // update 1000条数据，update操作会产生"BEFORE_UPDATE"和"AFTER_UPDATE"两条binlog；一共会有3000条binlog，
                // binlogIgnoreBeforeUpdate=true，只读取2000条
                for (int i = 0; i < 1000; ++i) {
                    Put put2 = new Put(schema);
                    put2.setObject("id", i);
                    put2.setObject("amount", "16.211");
    ...
    /** binlogGroupShardReader. delete and binlogIgnoreDelete=false */
    @Test
    ...
    /** binlogGroupShardReader. delete and binlogIgnoreDelete=true */
    @Test
    ...
    /** binlogGroupShardReader. Alter table: add one new column. */
    @Ignore
    @Test
        ...
        try (Connection conn = buildConnection();
            ...
            String createSql =
                    "create table "
                            ...
                            + "with (binlog_level='replica', table_group='tg_3');\n";
            String alterSql =
                    "ALTER TABLE IF EXISTS " + tableName + " ADD COLUMN new_column_1 int;\n";

            execute(conn, new String[] {dropSql1});
            execute(conn, new String[] {"begin;", createSql, "commit;"});
            try {
                try (BinlogShardGroupReader reader =
                        client.binlogSubscribe(Subscribe.newOffsetBuilder(tableName).build())) {
                    final CompletableFuture<Void> writeFuture = new CompletableFuture<>();
                    new Thread(
                                    () -> {
                                        try {
                                            TableSchema schema =
                                                    client.getTableSchema(tableName, true);
                    ...
                    while ((record = reader.getBinlogRecord()) != null) {
                        ...
                        if (id < 2000) {
                            ...
                            Assert.assertEquals(
                                    record.getSchema().getColumnSchema().length,
                                    8,
                                    record.getObject(0) + ":" + record.getClass().getName());
                            Assert.assertEquals((int) record.getObject(7), id);
                        }

                        // shard count不超过100
                        Assert.assertTrue(
                                set.add(record.getBinlogLsn() * 100 + record.getShardId()));
    ...
    /** binlogGroupShardReader. Alter table: drop one column. */
    @Ignore
    @Test
        ...
        try (Connection conn = buildConnection();
            ...
            String createSql =
                    "create table "
                            ...
                            + "with (binlog_level='replica', table_group='tg_3');\n";
            String alterSql =
                    "set hg_experimental_enable_drop_column=true;\n"
    ...
    /** binlogGroupShardReader. 单shard表 start_lsn 测试 */
    @Test
    ...
    /** binlogGroupShardReader. 多shard表 BinlogOffset 仅设置timestamp 测试 */
    @Test
        ...
        try (Connection conn = buildConnection();
            ...
            try {
                ...
                for (int i = 0; i < shardCount; i++) {
                    ...
                    offsetBuilder.addShardStartOffset(
                            i, new BinlogOffset(-1, now.getTime() * 1000L));
                }
    ...
    /** binlogGroupShardReader. 多shard表 BinlogOffset 测试 */
    @Test
            ...
            try {
                ...
                reader =
                        client.binlogSubscribe(
                                Subscribe.newOffsetBuilder(tableName)
                                        .addShardsStartOffset(
                                                lsnExpects.keySet(), new BinlogOffset())
                                        .build());
                ...
                while ((record = reader.getBinlogRecord()) != null) {
                    ...
                    offsetMap.replace(
                            record.getShardId(),
                            new BinlogOffset(record.getBinlogLsn(), record.getBinlogTimestamp()));
                    if (count == 25000) {
                ...
                try (HoloClient client2 = new HoloClient(config)) {
                    ...
                    while ((record = reader.getBinlogRecord()) != null) {
                        ...
                        offsetMap.replace(
                                record.getShardId(),
                                new BinlogOffset(
                                        record.getBinlogLsn(), record.getBinlogTimestamp()));
                        if (count == 50000) {
                            reader.cancel();
                            break;
                        }
                    ...
                    for (int i = 0; i < shardCount; i++) {
                        Assert.assertEquals(
                                lsnExpects.get(i), Long.valueOf(offsetMap.get(i).getSequence()));
                    }
                }
            } finally {
                if (reader != null) {
    ...
    /** binlogGroupShardReader. 多shard表 BinlogOffset 测试, 从holo保存的位点恢复, 需要创建slot */
    @Test
        ...
        try (Connection conn = buildConnection();
            ...
            String dropSql1 =
                    "drop table if exists "
                            ...
                            + ";\n";
            String dropSql2 =
                    "delete from hologres.hg_replication_progress where slot_name='"
                            + slotName
                            + "';\n";
            ...
            String createSql4 =
                    "call hg_create_logical_replication_slot('"
                            + slotName
                            + "', 'hgoutput', '"
                            + publicationName
                            + "');\n";
                ...
                while ((record = reader.getBinlogRecord()) != null) {
                    ...
                    if (count == 25000) {
                        ...
                        for (int shardId : offsetMap.keySet()) {
                            reader.commitFlushedLsn(
                                    shardId, offsetMap.get(shardId).getSequence(), 5000L);
                        }
                        Thread.sleep(20000);
                        reader.cancel();
                        break;
                                    ...
                                    Subscribe.newOffsetBuilder(tableName, slotName)
                                            ...
                                            .addShardsStartOffset(
                                                    IntStream.range(0, shardCount)
                                                            .boxed()
                                                            .collect(Collectors.toSet()),
                                                    new BinlogOffset().setTimestamp(null))
                                            .build());
                    while ((record = reader.getBinlogRecord()) != null) {
                        count++;
                        offsetMap.replace(
    ...
    /** binlogGroupShardReader. 用户自定义schema下的表 */
    @Test
    ...
    /** binlogGroupShardReader. 单shard表 restart 测试 */
    @Test
            ...
            try {
                ...
                while (true) {
                    ...
                    if ((record = reader.getBinlogRecord()) != null) {
                        count++;
                        // System.out.println("count: " + count + " shardId: " + record.getShardId()
                        // + " record lsn : " + record.getBinlogLsn());
    ...
    /** binlogGroupShardReader. 实例版本大于2.1时，不需要传入slot */
    @Test
        ...
        try (Connection conn = buildConnection();
            ...
            try {
                ...
                if (!(holoVersion.compareTo(new HoloVersion("2.1.0")) < 0
                        && e.getMessage()
                                .contains(
                                        "For hologres instance version lower than r2.1.0, need to provide slotName to subscribe binlog. your version is HoloVersion"))) {
    ...
    private String getPartitionTableName(String parentTable, Object partitionValue) {
        ...
        return TableName.valueOf(
                        ...
                        IdentifierUtil.quoteIdentifier(
                                parentName.getTableName() + "_" + partitionValue, true))
                .getFullName();
    }

    /** BinlogPartitionGroupReader. 读取分区父表的binlog DYNAMIC模式,指定一个很早的时间开始消费 */
    @Test
    public void binlogReaderPartitionTableDynamic001() throws Exception {
        if (properties == null || holoVersion.compareTo(needVersion) < 0) {
            return;
        }
        boolean testProjection = holoVersion.compareTo(new HoloVersion(3, 2, 0)) >= 0;
        boolean testCompression = holoVersion.compareTo(new HoloVersion(3, 2, 0)) >= 0;
        HoloConfig config = buildConfig();
        config.setOnConflictAction(OnConflictAction.INSERT_OR_REPLACE);
        config.setBinlogReadBatchSize(128);
        ...
        for (int recordCount : recordCountArray) {
            try (Connection conn = buildConnection();
                ...
                try {
                    ...
                    ZonedDateTime old =
                            ZonedDateTime.now(schema.getAutoPartitioning().getTimeZoneId())
                                    .minusHours(partitionCount - 1);
                    // 多创建一张子表，防止测试在时间单元切换时触发
                    for (int i = 0; i <= partitionCount; ++i) {
                        dt[i] =
                                Integer.parseInt(
                                        PartitionUtil.getPartitionSuffixByDateTime(
                                                old, schema.getAutoPartitioning()));
                        old = old.plusHours(1);
                    }
                        ...
                        for (int i = 0; i < partitionCount; ++i) {
                            ...
                            for (int shardId = 0; shardId < shardCount; shardId++) {
                                ...
                                if (rs.next() && rs.getLong(2) > 0) {
                                    expectTableShardMaxLsn.put(
                                            new Tuple<>(table, shardId), rs.getLong(2));
                                }
                            }
                        }
                    }
                            ...
                            Subscribe.newOffsetBuilder(tableName)
                                    .addShardsStartOffset(
                                            IntStream.range(0, shardCount)
                                                    .boxed()
                                                    .collect(Collectors.toSet()),
                                            new BinlogOffset().setTimestamp("2024-07-11 10:00:00"))
                                    .setEnableCompression(testCompression)
                                    .addProjectionColumnNamesToSubscribe(
                                            testProjection
                                                    ? new String[] {"id", "dt", "name", "ba"}
                                                    : null)
                                    .build();
                    ...
                    while ((record = reader.getBinlogRecord()) != null) {
                        ...
                        if (!(record instanceof BinlogHeartBeatRecord)) {
                            if (testProjection) {
                                ...
                                Assert.assertEquals(record.getBitSet().cardinality(), 8);
                            }
                            int id = (int) record.getObject("id");
                            Assert.assertEquals(
                                    record.getObject("dt"), String.valueOf(dt[id / recordCount]));
                            Assert.assertEquals(record.getObject("name"), "abc,d");
                            Assert.assertEquals(
                                    record.getObject("ba"), new byte[] {(byte) (id % 128)});
                        ...
                        if (count == (partitionCount + 1) * recordCount) {
                            ...
                            if (isMapContained(tableShardMaxLsn, expectTableShardMaxLsn)) {
                                reader.cancel();
                                Set<String> extraTable =
                                        mapSubtract(tableShardMaxLsn, expectTableShardMaxLsn)
                                                .keySet().stream()
                                                .map(a -> a.l)
                                                .collect(Collectors.toSet());
                                if (extraTable.size() != 1
                                        || !extraTable
                                                .iterator()
                                                .next()
                                                .equals(
                                                        getPartitionTableName(
                                                                tableName, dt[partitionCount]))) {
                                    Assert.fail("should not happen: bad result");
                                }
                                LOG.info(
                                        "reader cancel because test ok: new partition started consuming during the test.");
                                break;
                            } else {
                                Assert.fail(
                                        "should not happen: tableShardMaxLsn does not contain expectTableShardMaxLsn");
                            }
                        }
                    }
                } finally {
    ...
    /** BinlogPartitionGroupReader. 读取分区父表的binlog DYNAMIC模式,指定一个时间从某张子表开始 */
    @Test
                ...
                try {
                    ...
                    while ((record = reader.getBinlogRecord()) != null) {
                        ...
                        if (!(record instanceof BinlogHeartBeatRecord)) {
                            ...
                            int id = (int) record.getObject("id");
                            Assert.assertEquals(
                                    record.getObject("dt"), String.valueOf(dt[id / 100]));
                            Assert.assertEquals(record.getObject("name"), "abc,d");
                            Assert.assertEquals(
                                    record.getObject("ba"), new byte[] {(byte) (id % 128)});
                        ...
                        // System.out.println(count + ": " + record.getTableName() + ": " +
                        // record.getShardId() + ", " + record.getBinlogLsn() + ", " +
                        // Arrays.toString(record.getValues()));
                        // 测试过程中，新的时间单元到来，多消费了一张分区
    ...
    /** BinlogPartitionGroupReader. 读取分区父表的binlog DYNAMIC模式,从两个指定了lsn的子表开始消费,模拟从checkpoint恢复 */
    @Test
    public void binlogReaderPartitionTableDynamic003() throws Exception {
        ...
        try (Connection conn = buildConnection();
            ...
            try {
                ...
                try {
                    Subscribe.OffsetBuilder builder =
                            Subscribe.newOffsetBuilder(tableName)
                                    .addShardsStartOffset(
                                            shards,
                                            new BinlogOffset().setTimestamp("2024-07-11 10:00:00"))
                    ...
                    for (int i = 0; i < 3; i++) {
                        ...
                        for (int j = 0; j < shardCount; j++) {
                            builder.addShardStartOffsetForPartition(
                                    table,
                                    j,
                                    new BinlogOffset()
                                            .setSequence(
                                                    expectTableShardMaxLsn.get(
                                                            new Tuple<>(
                                                                    getPartitionTableName(
                                                                            tableName, dt[i]),
                                                                    j))));
                ...
                while ((record = reader.getBinlogRecord()) != null) {
                    ...
                    if (!(record instanceof BinlogHeartBeatRecord)) {
                        ...
                        int id = (int) record.getObject("id");
                        Assert.assertEquals(record.getObject("dt"), String.valueOf(dt[id / 100]));
                        Assert.assertEquals(record.getObject("name"), "abc,d");
                        Assert.assertEquals(record.getObject("ba"), new byte[] {(byte) (id % 128)});
                        count++;
                        tableShardMaxLsn.put(new Tuple<>(table, shardId), lsn);
                    }
                    System.out.println(
                    ...
                    if (count == 2 * 200 + 4 * 300) {
                        ...
                        if (isMapContained(tableShardMaxLsn, expectTableShardMaxLsn)) {
                            reader.cancel();
                            Set<String> extraTable =
                                    mapSubtract(tableShardMaxLsn, expectTableShardMaxLsn).keySet()
                                            .stream()
                                            .map(a -> a.l)
                                            .collect(Collectors.toSet());
                            if (extraTable.size() != 1
    ...
    /** BinlogPartitionGroupReader. 读取分区父表的binlog DYNAMIC模式,从某个指定了lsn的子表开始消费,但表已经不存在 */
    @Test
        ...
        try (Connection conn = buildConnection();
            ...
            try {
                ...
                try {
                    ...
                    if (!e.getMessage()
                            .contains(
                                    "it may be an earlier partition table that has been dropped, could not start subscribe from lsn")) {
                        throw e;
                    }
                }
            } finally {
     ...
     */
    @Test
    public void binlogReaderPartitionTableDynamic005() throws Exception {
        if (properties == null || holoVersion.compareTo(needVersion) < 0) {
            return;
        }
        if (holoVersion.compareTo(new HoloVersion(3, 0, 12)) < 0) {
            return;
        }
        boolean testProjection = holoVersion.compareTo(new HoloVersion(3, 2, 0)) >= 0;
        boolean testCompression = holoVersion.compareTo(new HoloVersion(3, 2, 0)) >= 0;
        ...
        try (Connection conn = buildConnection();
                HoloClient client = new HoloClient(config)) {
            String tableName = "\"TEST\".\"test_BINLOG_partition_table_DYNAMIC_005\"";
            String createSchema = "create schema if not exists \"TEST\"";
            String dropSql1 = "drop table if exists " + tableName;
            String createSql =
                    String.format(
                            "create table %s"
                                    + "(id int not null, amount decimal(12,2), name text, dt text, ts timestamptz, ba bytea, t_a text[],i_a int[], primary key(id, dt)) "
                                    + "PARTITION BY LIST(dt) "
                                    + "with (binlog_level='replica', table_group='tg_3', auto_partitioning_enable='true', auto_partitioning_time_unit='HOUR', auto_partitioning_time_format='YYYY-MM-DD-HH24');\n ",
                            tableName);
            execute(conn, new String[] {createSchema, dropSql1});
            execute(conn, new String[] {createSql});
            ...
            try {
                TableSchema schema = client.getTableSchema(tableName, true);
                // 除了当前子表，同时创建符合动态分区命名方式的（partitionCount -1 )个历史子表, 和未来的一张子表
                String[] dt = new String[partitionCount + 1];
                ZonedDateTime old =
                        ZonedDateTime.now(schema.getAutoPartitioning().getTimeZoneId())
                                .minusHours(partitionCount - 1);
                // 多创建一张子表，防止测试在时间单元切换时触发
    ...
    /** BinlogPartitionGroupReader. 读取分区父表的binlog,默认读取所有子表binlog 为每个需要消费的子表指定offsetMap */
    @Test
            ...
            try {
                ...
                while ((record = reader.getBinlogRecord()) != null) {
                    ...
                    if (!(record instanceof BinlogHeartBeatRecord)) {
                        ...
                        int id = (int) record.getObject("id");
                        Assert.assertEquals(record.getObject("pt"), "kind_" + id / 100);
                        Assert.assertEquals(record.getObject("name"), "abc,d");
                        Assert.assertEquals(record.getObject("ba"), new byte[] {(byte) (id % 128)});
                        count++;
                        tableShardMaxLsn.put(new Tuple<>(table, shardId), lsn);
                    ...
                    if (count == partitionCount * 100) {
                        Assert.assertEquals(
                                reader.getCurrentPartitionsInSubscribe().size(), partitionCount);
                        reader.cancel();
                        break;
                    }
                }
                ...
                for (int i = 0; i < partitionCount - 1; ++i) {
                    ...
                    for (int shardId = 0; shardId < shardCount; shardId++) {
                        builder.addShardStartOffset(
                                        shardId,
                                        new BinlogOffset()
                                                .setSequence(
                                                        expectTableShardMaxLsn.get(
                                                                new Tuple<>(table, shardId))))
                                .setEnableCompression(testCompression)
                                .addProjectionColumnNamesToSubscribe(
                                        testProjection
                                                ? new String[] {"id", "pt", "name", "ba"}
                ...
                while ((record = reader.getBinlogRecord()) != null) {
                    ...
                    if (count == (partitionCount - 1) * 100 * 2) {
                        Assert.assertEquals(
                                reader.getCurrentPartitionsInSubscribe().size(),
                                partitionCount - 1);
                        reader.cancel();
                        break;
                    }
    ...
    /** BinlogPartitionGroupReader. 读取分区父表的binlog, 通过参数指定部分子表 */
    @Test
    public void binlogReaderPartitionTableStatic002() throws Exception {
        ...
        try (Connection conn = buildConnection();
            ...
            try {
                ...
                Subscribe.OffsetBuilder offsetBuilder =
                        Subscribe.newOffsetBuilder(tableName)
                                .addPartitionValuesToSubscribe(
                                        new String[] {"kind_1", "kind_2", "kind_3", "kind_4"})
                                .setEnableCompression(testCompression)
                                .addProjectionColumnNamesToSubscribe(
                                        testProjection
                                                ? new String[] {"id", "pt", "name", "ba"}
    ...
    /** BinlogPartitionGroupReader. 读取分区父表的binlog, 通过参数指定不存在的分区值,期望抛出异常 */
    @Test
        ...
        try (Connection conn = buildConnection();
            ...
            try {
                ...
                Subscribe.OffsetBuilder offsetBuilder =
                        Subscribe.newOffsetBuilder(tableName)
                                .addPartitionValuesToSubscribe(
                                        new String[] {"kind_1", "kind_2", "kind_3", "kind_488"})
                                .setEnableCompression(testCompression)
                                .addProjectionColumnNamesToSubscribe(
                                        testProjection
                                                ? new String[] {"id", "pt", "name", "ba"}
                ...
                try {
                    ...
                    if (!e.getMessage()
                            .contains(
                                    "partition value kind_488 not found in partitioned table Test_Binlog_partition_table_STATIC_003")) {
                        throw e;
                    }
                }
            } finally {
    ...
    /** BinlogPartitionGroupReader. 读取分区父表的binlog, 通过参数指定部分子表的分区值. 父表指定起始消费时间 */
    @Test
    public void binlogReaderPartitionTableStatic004() throws Exception {
        ...
        boolean[] offsetBuilderType = new boolean[] {false, true};
        for (Boolean useOffsetBuilder : offsetBuilderType) {
            int shardCount = 3;
            int partitionCount = 5;
            try (Connection conn = buildConnection();
                    HoloClient client = new HoloClient(config)) {
                ...
                try {
                    ...
                    if (useOffsetBuilder) {
                        ...
                        for (int shardId = 0; shardId < shardCount; shardId++) {
                            offsetBuilder.addShardStartOffset(
                                    shardId, new BinlogOffset().setTimestamp(startTime));
                        }
                        parentSubscribe = offsetBuilder.build();
                    } else {
                        Subscribe.StartTimeBuilder startTimeBuilder =
                                Subscribe.newStartTimeBuilder(tableName)
                                        .addPartitionValuesToSubscribe(
                                                new String[] {
                                                    "kind_1", "kind_2", "kind_3", "kind_4"
                                        ...
                                        .addProjectionColumnNamesToSubscribe(
                                                testProjection
                                                        ? new String[] {"id", "pt", "name", "ba"}
                                                        : null);
                        startTimeBuilder.setBinlogReadStartTime(startTime);
                        parentSubscribe = startTimeBuilder.build();
                    }
                    reader = client.partitionBinlogSubscribe(parentSubscribe);
    ...
    /**
     * Read binlog for logical partition table, set partition_column_names_to_subscribe and
     * partition_values_to_subscribe to filter.
     */
    @Test
    public void binlogReaderLogicalPartitionTable001() throws Exception {
        if (properties == null || holoVersion.compareTo(new HoloVersion(3, 1, 0)) < 0) {
            return;
        }
        boolean testProjection = holoVersion.compareTo(new HoloVersion(3, 2, 0)) >= 0;
        boolean testCompression = holoVersion.compareTo(new HoloVersion(3, 2, 0)) >= 0;
        ...
        for (Boolean useOffsetBuilder : offsetBuilderType) {
            try (Connection conn = buildConnection();
                    HoloClient client = new HoloClient(config)) {
                String createSchema = "create schema if not exists \"Test\"";
                String tableName = "\"Test\".\"Test_Binlog_logical_partition_table_001\"";
                String dropSql1 = "drop table if exists " + tableName;
                String createSql =
                        String.format(
                                "create table %s"
                                        + "(id int not null, amount decimal(12,2), event_date date, name text, pt text, ts timestamptz, ba bytea, t_a text[],i_a int[], primary key(id, event_date, pt)) "
                                        + "LOGICAL PARTITION BY LIST(event_date, pt) "
                                        + "with (binlog_level='replica', table_group='tg_3');\n ",
                                tableName);
                execute(conn, new String[] {createSchema, dropSql1});
                execute(conn, new String[] {createSql});
                ...
                try {
                    ...
                    for (int i = 0; i < partitionCount * 100; ++i) {
                        ...
                        put.setObject("id", i);
                        put.setObject("amount", "16.211");
                        put.setObject("event_date", "2024-11-01");
                        put.setObject("name", "abc,d");
                        int pt = i / 100;
                        put.setObject("pt", "kind_" + pt);
                        put.setObject("ts", "2024-11-01 12:12:12");
                        put.setObject("ba", new byte[] {(byte) (i % 128)});
                        put.setObject("t_a", new String[] {"a", "b,c"});
                        put.setObject("i_a", new int[] {1, 2, 3, 4, 5});
                        client.put(put);
                    ...
                    Map<Integer, Long> expectShardMaxLsn = new HashMap<>();
                        ...
                        for (int shardId = 0; shardId < shardCount; shardId++) {
                            String query =
                                    "SELECT "
                                            ...
                                            + "            AND event_date = '2024-11-01'));";
                            ResultSet rs =
                                    stat.executeQuery(String.format(query, tableName, shardId));
                            if (rs.next() && rs.getLong(1) != 0) {
                                expectShardMaxLsn.put(shardId, rs.getLong(1));
                            }
                        }
                    }
                    Subscribe logicalPartitionSubscribe;
                    try {
                        Subscribe.OffsetBuilder offsetBuilder =
                                Subscribe.newOffsetBuilder(tableName)
                                        .addLogicalPartitionNamesToSubscribe(
                                                new String[] {"event_date", "pt"})
                                        .addLogicalPartitionValuesToSubscribe(
                                                new String[][] {
                                                    {"2024-11-01", "kind_1"},
                                                    {"2024-11-01", "kind_2"},
                                                    {"2024-11-01", "kind_3"}
                        ...
                        logicalPartitionSubscribe = offsetBuilder.build();
                        reader = client.binlogSubscribe(logicalPartitionSubscribe);
                        reader.getBinlogRecord();
                    } catch (HoloClientException e) {
                        if (!e.getCause()
                                .getMessage()
                                .contains(
                                        "logical partition column event_date is not in projection column names")) {
                            throw e;
                        }
                    }
                    if (useOffsetBuilder) {
                                ...
                                Subscribe.newStartTimeBuilder(tableName)
                                        ...
                                        .addProjectionColumnNamesToSubscribe(
                                                testProjection
                                                        ...
                                                        : null);
                        startTimeBuilder.setBinlogReadStartTime(startTime);
                        logicalPartitionSubscribe = startTimeBuilder.build();
                    }

                    reader = client.binlogSubscribe(logicalPartitionSubscribe);

                    Map<Integer, Long> shardMaxLsn = new HashMap<>();
                    ...
                    while ((record = reader.getBinlogRecord()) != null) {
                        ...
                        if (!(record instanceof BinlogHeartBeatRecord)) {
                            if (testProjection) {
                                Assert.assertEquals(record.getBitSet().cardinality(), 5);
                            } else {
                                Assert.assertEquals(record.getBitSet().cardinality(), 9);
                            }
                            int id = (int) record.getObject("id");
                            Assert.assertEquals(record.getObject("pt"), "kind_" + id / 100);
                            Assert.assertEquals(record.getObject("name"), "abc,d");
                    ...
                    Assert.assertEquals(
                            expectShardMaxLsn,
                            shardMaxLsn,
                            "Values do not match: expectShardMaxLsn="
                                    + expectShardMaxLsn
                                    + ", shardMaxLsn="
                                    + shardMaxLsn);
                } finally {
    ...
    /** Read binlog for logical partition table, the partition column includes generated column. */
    @Test
    public void binlogReaderLogicalPartitionTable002() throws Exception {
        if (properties == null || holoVersion.compareTo(new HoloVersion(3, 1, 0)) < 0) {
            return;
        }
        boolean testProjection = holoVersion.compareTo(new HoloVersion(3, 2, 0)) >= 0;
        ...
        for (Boolean useOffsetBuilder : offsetBuilderType) {
            ...
            try (Connection conn = buildConnection();
                    HoloClient client = new HoloClient(config)) {
                String createSchema = "create schema if not exists \"Test\"";
                String tableName = "\"Test\".\"Test_Binlog_logical_partition_table_002\"";
                String dropSql1 = "drop table if exists " + tableName;
                String createSql =
                        String.format(
                                "create table %s"
                                        + "(id int not null, amount decimal(12,2), event_date date, name text, pt text, ts timestamp, "
                                        + "ts_generated timestamp generated always as (ts) stored, ts_tz timestamp, ts_tz_generated timestamptz generated always as (ts_tz) stored, "
                                        + "ba bytea, t_a text[],i_a int[], "
                                        + "primary key(id, event_date, pt, ts_generated, ts_tz_generated))"
                                        + "LOGICAL PARTITION BY LIST(ts_generated, ts_tz_generated) "
                                        + "with (binlog_level='replica', table_group='tg_3', distribution_key='id');\n ",
                                tableName);
                execute(conn, new String[] {createSchema, dropSql1});
                execute(conn, new String[] {createSql});
                BinlogShardGroupReader reader = null;
                ...
                try {
                    ...
                    for (int i = 0; i < partitionCount * 100; ++i) {
                        ...
                        int pt = i / 100;
                        put.setObject("pt", "kind_" + pt);
                        put.setObject("ts", "2024-11-0" + pt + " 12:12:12");
                        put.setObject("ts_tz", "2024-11-0" + pt + " 12:12:12 +08");
                        put.setObject("ba", new byte[] {(byte) (i % 128)});
                        put.setObject("t_a", new String[] {"a", "b,c"});
                        put.setObject("i_a", new int[] {1, 2, 3, 4, 5});
                        client.put(put);
                    ...
                    if (useOffsetBuilder) {
                        Subscribe.OffsetBuilder offsetBuilder =
                                Subscribe.newOffsetBuilder(tableName)
                                        .addLogicalPartitionNamesToSubscribe(
                                                new String[] {"ts_generated", "ts_tz_generated"})
                                        .addLogicalPartitionValuesToSubscribe(
                                                new String[][] {
                                                    {
                                                        "2024-11-01 12:12:12",
                                                        "2024-11-01 12:12:12 +08"
                                                    },
                                                    {
                                                        "2024-11-02 12:12:12",
                                                        "2024-11-02 12:12:12 +08"
                                                    },
                                                    {
                                                        "2024-11-03 12:12:12",
                                                        "2024-11-03 12:12:12 +08"
                                                    }
                                                })
                                        .setEnableCompression(testCompression)
                                        .addProjectionColumnNamesToSubscribe(
                    ...
                    while ((record = reader.getBinlogRecord()) != null) {
                        ...
                        if (!(record instanceof BinlogHeartBeatRecord)) {
                            if (testProjection) {
                                Assert.assertEquals(record.getBitSet().cardinality(), 6);
                            } else {
                                Assert.assertEquals(record.getBitSet().cardinality(), 12);
                            }
                            int id = (int) record.getObject("id");
                            Assert.assertEquals(record.getObject("pt"), "kind_" + id / 100);
                            Assert.assertEquals(record.getObject("name"), "abc,d");
                            Assert.assertEquals(
                                    record.getObject("ba"), new byte[] {(byte) (id % 128)});
                            Assert.assertEquals(
                                    record.getObject("ts_generated").toString(),
                                    "2024-11-0" + id / 100 + " 12:12:12.0");
                            Assert.assertEquals(
                                    record.getObject("ts_tz_generated").toString(),
                                    "2024-11-0" + id / 100 + " 12:12:12.0");
                            count++;
                            shardMaxLsn.put(shardId, lsn);
                        }
    ...
    /**
     * Read binlog for logical partition table, partition_columns_to_subscribe include a subset of
     * columns.
     */
    @Test
    public void binlogReaderLogicalPartitionTable003() throws Exception {
        if (properties == null || holoVersion.compareTo(new HoloVersion(3, 1, 0)) < 0) {
            return;
        }
        boolean testProjection = holoVersion.compareTo(new HoloVersion(3, 2, 0)) >= 0;
        ...
        for (Boolean useOffsetBuilder : offsetBuilderType) {
            ...
            try (Connection conn = buildConnection();
                    HoloClient client = new HoloClient(config)) {
                String createSchema = "create schema if not exists \"Test\"";
                String tableName = "\"Test\".\"Test_Binlog_logical_partition_table_003\"";
                String dropSql1 = "drop table if exists " + tableName;
                String createSql =
                        String.format(
                                "create table %s"
                                        + "(id int not null, amount decimal(12,2), event_date date, name text, pt text, ts timestamp, ts_generated timestamp generated always as (ts) stored, ts_tz timestamptz, ts_tz_generated timestamptz generated always as (ts_tz) stored, ba bytea, t_a text[],i_a int[], primary key(id, event_date, pt)) "
                                        + "LOGICAL PARTITION BY LIST(event_date, pt) "
                                        + "with (binlog_level='replica', table_group='tg_3');\n ",
                                tableName);
                execute(conn, new String[] {createSchema, dropSql1});
                ...
                try {
                    ...
                    if (useOffsetBuilder) {
                        Subscribe.OffsetBuilder offsetBuilder =
                                Subscribe.newOffsetBuilder(tableName)
                                        .addLogicalPartitionNamesToSubscribe(new String[] {"pt"})
                                        .addLogicalPartitionValuesToSubscribe(
                                                new String[][] {{"kind_1"}, {"kind_2"}, {"kind_3"}})
                                        .setEnableCompression(testCompression)
                                        .addProjectionColumnNamesToSubscribe(
                                                testProjection
                                                        ? new String[] {"id", "pt", "name", "ba"}
    ...
    public Object[][] createDataForReadBinlog() {
        ...
        for (int i = 0; i < typeToTest.length; ++i) {
            ret[i] = new Object[] {typeToTest[i], supportCompression};
        }
        return ret;
    }
    ...
    public void binlogReaderDataTypeTest(
            DataTypeTestUtil.TypeCaseDataWithRecord typeCaseData, boolean supportCompression)
            throws Exception {
        if (properties == null || holoVersion.compareTo(needVersion) < 0) {
            return;
        }
        ...
        try (Connection conn = buildConnection();
            ...
            if (typeName.equals("jsonb")) {
                // set "alter database xx set hg_experimental_enable_binlog_jsonb = on" in test
                // instance to support jsonb.
            ...
            try {
                ...
                reader =
                        client.binlogSubscribe(
                                Subscribe.newOffsetBuilder(tableName)
                                        .setEnableCompression(supportCompression)
                                        .addShardsStartOffset(
                                                IntStream.range(0, 1)
                                                        .boxed()
                                                        .collect(Collectors.toSet()),
    ...
    private boolean isBinlogFilterSupported(
        ...
        switch (typeName) {
            ...
            case "text":
                return true;
            default:
                if (operatorType == ServiceContractMsg.OperatorType.IS_NULL
                        || operatorType == ServiceContractMsg.OperatorType.IS_NOT_NULL) {
                    return true;
                }
        }
        return false;
    ...
    public static <K, V> boolean isMapContained(Map<K, V> largeMap, Map<K, V> smallMap) {
        ...
        for (Map.Entry<K, V> entry : smallMap.entrySet()) {
            if (!largeMap.containsKey(entry.getKey())
                    || !largeMap.get(entry.getKey()).equals(entry.getValue())) {
                return false;
            }
        }
        return true;
