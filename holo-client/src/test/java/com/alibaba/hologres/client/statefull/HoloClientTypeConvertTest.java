		...
		try (Connection conn = buildConnection(); HoloClient client = new HoloClient(config)) {
			...
			try {
				...
				for (int i = 0; i < inputAndExpectList.length; ++i) {
					...
					Record r = client.get(get).get();
					Assert.assertEquals(pair[1], formatter.format(new java.util.Date(((Timestamp) r.getObject(1)).getTime())), pair[0]);
					Assert.assertEquals(pair[1], formatter.format(new java.util.Date(((Timestamp) r.getObject(2)).getTime())), pair[0]);
				}
		...
		try (Connection conn = buildConnection(); HoloClient client = new HoloClient(config)) {
			...
			try {
				...
				for (int i = 0; i < inputList.length; ++i) {
					...
					Record r = client.get(get).get();
					Assert.assertEquals(expectList[i], formatter.format(new java.util.Date(((Timestamp) r.getObject(1)).getTime())), "" + inputList[i]);
					Assert.assertEquals(expectList[i], formatter.format(new java.util.Date(((Timestamp) r.getObject(2)).getTime())), "" + inputList[i]);
				}
		...
		try (Connection conn = buildConnection(); HoloClient client = new HoloClient(config)) {
			...
			try {
				...
				for (int i = 0; i < inputList.length; ++i) {
					...
					Record r = client.get(get).get();
					Assert.assertEquals(expectList[i], formatter.format((Date) r.getObject(1)), "" + inputList[i]);
				}
		...
		try (Connection conn = buildConnection(); HoloClient client = new HoloClient(config)) {
			...
			try {
				...
				for (int i = 0; i < inputList.length; ++i) {
					...
					Record r = client.get(get).get();
					Assert.assertEquals(expectList[1], formatter.format((Date) r.getObject(1)), "" + inputList[0]);
				}
