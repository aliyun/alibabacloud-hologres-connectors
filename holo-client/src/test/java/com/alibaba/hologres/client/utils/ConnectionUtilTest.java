...
public class ConnectionUtilTest extends HoloClientTestBase {
	...
	public void testMetaDataGetColumns() throws Exception {
		...
		try (Connection conn = buildConnection(); HoloClient client = new HoloClient(config)) {
			...
			String createSql1 = "create table " + tableName1 + "(a int not null,b int,primary key(a))";
			...
			String createSql2 = "create table " + tableName2 + "(c text not null,d text,primary key(c))";
			...
			String createSql4 = "create table " + tableName4 + "(g boolean,h boolean,primary key(g))";
			...
			String createSql5 = "create table " + tableName5 + "(i timestamptz,j timestamptz,primary key(i))";
			execute(conn, new String[]{createSchema5, dropSql5, createSql5});

			try {
				Column[] table1Columns = ConnectionUtil.getTableSchema(conn, TableName.valueOf(tableName1)).getColumnSchema();
				...
				Column[] table2Columns = ConnectionUtil.getTableSchema(conn, TableName.valueOf(tableName2)).getColumnSchema();
				...
				Column[] table3Columns = ConnectionUtil.getTableSchema(conn, TableName.valueOf(tableName3)).getColumnSchema();
				...
				Column[] table4Columns = ConnectionUtil.getTableSchema(conn, TableName.valueOf(tableName4)).getColumnSchema();
				...
				Column[] table5Columns = ConnectionUtil.getTableSchema(conn, TableName.valueOf(tableName5)).getColumnSchema();
