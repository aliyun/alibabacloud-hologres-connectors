...
public class HoloClientScanTest extends HoloClientTestBase {
	...
	public void testScan001() throws Exception {
		...
		try (Connection conn = buildConnection(); HoloClient client = new HoloClient(config)) {
			...
			try {
				TableSchema schema = client.getTableSchema(tableName);
				Assert.assertEquals(schema.getClusteringKey(), new String[]{"\"c1:asc\":asc", "\"c2:desc\":desc", "\"C:3\":asc"});
				for (int i = 0; i < 3; i++) {
					Put put = new Put(schema);
					put.setObject(0, "123456");
					put.setObject(1, i);
				...
				try (RecordScanner scanner = client.scan(Scan.newBuilder(schema).setSortKeys(SortKeys.PRIMARY_KEY)
						.build())) {
					int count = 0;
					while (scanner.next()) {
						Record r = scanner.getRecord();
				...
				try (RecordScanner scanner = client.scan(Scan.newBuilder(schema).setSortKeys(SortKeys.CLUSTERING_KEY)
						.build())) {
					int count = 0;
					while (scanner.next()) {
						Record r = scanner.getRecord();
