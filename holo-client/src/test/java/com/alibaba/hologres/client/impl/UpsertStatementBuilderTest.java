...
public class UpsertStatementBuilderTest extends HoloClientTestBase {
    ...
    public void testBuildCheckAndPutPattern() {
        ...
        for (CheckCompareOp op : CheckCompareOp.values()) {
            ...
            CheckAndPutCondition checkAndPutCondition1 =
                    new CheckAndPutCondition(checkColumn1, op, null, null);
            if (op == CheckCompareOp.IS_NULL || op == CheckCompareOp.IS_NOT_NULL) {
                // 已有数据与null做比较：old column is null
                Assert.assertEquals(
                        " old.\"TEST_Column\" " + op.getOperatorString(),
                        builder.buildCheckAndPutPattern(checkAndPutCondition1));
            } else {
                Assert.assertEquals(
                        " excluded.\"TEST_Column\" "
                                + op.getOperatorString()
                                + " old.\"TEST_Column\"",
                        builder.buildCheckAndPutPattern(checkAndPutCondition1));
            }
        }
        ...
        for (CheckCompareOp op : CheckCompareOp.values()) {
            ...
            checkColumn1.setTypeName("bigint");
            CheckAndPutCondition checkAndPutCondition1 =
                    new CheckAndPutCondition(checkColumn1, op, null, 0L);
            if (op == CheckCompareOp.IS_NULL || op == CheckCompareOp.IS_NOT_NULL) {
                Assert.assertEquals(
                        " coalesce(old.\"TEST_Column\", '0'::bigint) " + op.getOperatorString(),
                        builder.buildCheckAndPutPattern(checkAndPutCondition1));
            } else {
                Assert.assertEquals(
                        " excluded.\"TEST_Column\" "
                                + op.getOperatorString()
                                + " coalesce(old.\"TEST_Column\", '0'::bigint)",
                        builder.buildCheckAndPutPattern(checkAndPutCondition1));
            }
        }
        ...
        for (CheckCompareOp op : CheckCompareOp.values()) {
            ...
            checkColumn1.setTypeName("bigint");
            CheckAndPutCondition checkAndPutCondition1 =
                    new CheckAndPutCondition(checkColumn1, op, 1234L, null);
            if (op == CheckCompareOp.IS_NULL || op == CheckCompareOp.IS_NOT_NULL) {
                // 已有数据与null做比较：old column is null
                Assert.assertEquals(
                        " old.\"TEST_Column\" " + op.getOperatorString(),
                ...
                Assert.assertEquals(
                        " '1234'::bigint " + op.getOperatorString() + " old.\"TEST_Column\"",
                        builder.buildCheckAndPutPattern(checkAndPutCondition1));
            }
        }
        ...
        for (CheckCompareOp op : CheckCompareOp.values()) {
            ...
            checkColumn1.setTypeName("bigint");
            CheckAndPutCondition checkAndPutCondition1 =
                    new CheckAndPutCondition(checkColumn1, op, 1234L, 0L);
            if (op == CheckCompareOp.IS_NULL || op == CheckCompareOp.IS_NOT_NULL) {
                Assert.assertEquals(
                        " coalesce(old.\"TEST_Column\", '0'::bigint) " + op.getOperatorString(),
                        builder.buildCheckAndPutPattern(checkAndPutCondition1));
    ...
    public void testBuildCheckAndDeletePattern() {
        ...
        for (CheckCompareOp op : CheckCompareOp.values()) {
            ...
            if (op == CheckCompareOp.IS_NULL || op == CheckCompareOp.IS_NOT_NULL) {
                // 已有数据与null做比较：old column is null
                Assert.assertEquals(
                        " \"TEST_Column\" " + op.getOperatorString(),
                        builder.buildCheckAndDeletePattern(checkAndPutCondition1));
            } else {
                Assert.assertEquals(
                        " ? " + op.getOperatorString() + " \"TEST_Column\"",
                        builder.buildCheckAndDeletePattern(checkAndPutCondition1));
            }
        }
        ...
        for (CheckCompareOp op : CheckCompareOp.values()) {
            ...
            if (op == CheckCompareOp.IS_NULL || op == CheckCompareOp.IS_NOT_NULL) {
                // 已有数据与null做比较：old column is null
                Assert.assertEquals(
                        " coalesce(\"TEST_Column\", '0'::bigint) " + op.getOperatorString(),
                        builder.buildCheckAndDeletePattern(checkAndPutCondition1));
            } else {
                Assert.assertEquals(
                        " ? " + op.getOperatorString() + " coalesce(\"TEST_Column\", '0'::bigint)",
                        builder.buildCheckAndDeletePattern(checkAndPutCondition1));
            }
        }
        ...
        for (CheckCompareOp op : CheckCompareOp.values()) {
            ...
            if (op == CheckCompareOp.IS_NULL || op == CheckCompareOp.IS_NOT_NULL) {
                ...
                Assert.assertEquals(
                        " '1234'::bigint " + op.getOperatorString() + " \"TEST_Column\"",
                        builder.buildCheckAndDeletePattern(checkAndPutCondition1));
            }
        }
