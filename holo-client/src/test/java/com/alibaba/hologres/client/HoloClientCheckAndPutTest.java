 ...
 *
 * @version 1.0
 * @since <pre>12月, 2023</pre>
 */
public class HoloClientCheckAndPutTest extends HoloClientTestBase {

	/**
	 * 两个参数：
	 * 第一个boolean表示使用values还是unnest方式拼sql
	 * 第二个boolean表示使用insertOrUpdate还是insertOrReplace.
	 */
	...
	public void testCheckAndPutException() throws Exception {
		...
		try (Connection conn = buildConnection(); HoloClient client = new HoloClient(config)) {
			...
			String createSql = "create table " + tableName + "(id int not null,name text,modify_time timestamptz,primary key(id))";

			execute(conn, new String[]{dropSql, createSql});

			TableSchema schema = client.getTableSchema(tableName);

			assertThrowsWithMessage(HoloClientException.class, "CheckAndPut not supports writeMode insertOrIgnore.", () -> {
				...
				try (HoloClient client1 = new HoloClient(config1)) {
					CheckAndPut put = new CheckAndPut(schema, "modify_time", CheckCompareOp.GREATER, null, "1970-01-01 00:08:00");
					put.setObject(0, 0);
			...
			assertThrowsWithMessage(HoloClientWithDetailsException.class, "checkColumn not_exist_column is not exists in table", () -> {
				CheckAndPut put = new CheckAndPut(schema, "not_exist_column", CheckCompareOp.GREATER_OR_EQUAL, null, "1970-01-01 00:08:00");
				put.setObject(0, 0);
				put.setObject(1, "name0");
				put.setObject(2, "2020-01-01 00:00:00");
			...
			assertThrowsWithMessage(HoloClientWithDetailsException.class, "checkColumn modify_time should be set when not set checkValue.", () -> {
				CheckAndPut put = new CheckAndPut(schema, "modify_time", CheckCompareOp.GREATER, null, "1970-01-01 00:08:00");
				put.setObject(0, 0);
				client.checkAndPut(put);
			});

			assertThrowsWithMessage(HoloClientWithDetailsException.class, "checkColumn modify_time should be set not null when not set checkValue and mutationType is delete.", () -> {
				CheckAndPut put = new CheckAndPut(schema, "modify_time", CheckCompareOp.GREATER, null, "1970-01-01 00:08:00");
				put.setObject(0, 0);
	...
	public void testCheckAndPutBatch002(boolean useLegacyPutHandler, boolean update) throws Exception {
		if (properties == null) {
			return;
		}
		HoloConfig config = buildConfig();
		config.setWriteThreadSize(1);
		config.setWriteMode(update ? WriteMode.INSERT_OR_UPDATE : WriteMode.INSERT_OR_REPLACE);
		...
		try (Connection conn = buildConnection(); HoloClient client = new HoloClient(config)) {
			...
			String dropSql = "drop table if exists " + tableName;
			String createSql = "create table " + tableName + "(id int not null,name text,modify_time timestamptz, address text,primary key(id))";
			...
			client.checkAndPut(put);
			put = new CheckAndPut(schema, "modify_time", CheckCompareOp.GREATER, null, "1970-01-01 00:08:00");
			put.setObject(0, 1);
	...
	/**
	 * INSERT
	 * 表中已存在的checkColumn字段是null，nullValue生效
	 * Method: checkAndPut(CheckAndPut put).
	 */
	@Test(dataProvider = "useLegacyPutHandler")
	public void testCheckAndPutBatch003(boolean useLegacyPutHandler, boolean update) throws Exception {
		if (properties == null) {
			return;
		}
		HoloConfig config = buildConfig();
		...
		try (Connection conn = buildConnection(); HoloClient client = new HoloClient(config)) {
			...
			CheckAndPut put = new CheckAndPut(schema, "modify_time", CheckCompareOp.GREATER, null, "2020-01-01 00:00:00");
			put.setObject(0, 0);
			put.setObject(1, "name0");
			put.setObject(2, null);
			put.setObject(3, "address0");
			client.checkAndPut(put);
			put = new CheckAndPut(schema, "modify_time", CheckCompareOp.GREATER, null, "2020-01-01 00:00:00");
			put.setObject(0, 1);
			put.setObject(1, "name1");
			put.setObject(2, null);
			put.setObject(3, "address1");
	...
	/**
	 * INSERT
	 * 传入了checkValue，相当于和常量比较
	 * Method: checkAndPut(CheckAndPut put).
	 */
	@Test(dataProvider = "useLegacyPutHandler")
	public void testCheckAndPutBatch004(boolean useLegacyPutHandler, boolean update) throws Exception {
		if (properties == null) {
			return;
		}
		HoloConfig config = buildConfig();
		...
		try (Connection conn = buildConnection(); HoloClient client = new HoloClient(config)) {
			...
			CheckAndPut put = new CheckAndPut(schema, "modify_time", CheckCompareOp.GREATER, "2020-01-01 00:00:00", "1970-01-01 00:08:00");
			put.setObject(0, 0);
			put.setObject(1, "name0");
			put.setObject(2, "2019-01-01 00:00:00");
			put.setObject(3, "address0");
			client.checkAndPut(put); // insert
			put = new CheckAndPut(schema, "modify_time", CheckCompareOp.GREATER, "2020-01-01 00:00:00", "1970-01-01 00:08:00");
			put.setObject(0, 1);
			put.setObject(1, "name1");
			put.setObject(2, null);
			put.setObject(3, "address1");
	...
	/**
	 * INSERT
	 * 传入了不同的checkValue，不会攒批
	 * Method: checkAndPut(CheckAndPut put).
	 */
	@Test(dataProvider = "useLegacyPutHandler")
	public void testCheckAndPutBatch005(boolean useLegacyPutHandler, boolean update) throws Exception {
		if (properties == null) {
			return;
		}
		HoloConfig config = buildConfig();
		...
		try (Connection conn = buildConnection(); HoloClient client = new HoloClient(config)) {
			...
			put = new CheckAndPut(schema, "modify_time", CheckCompareOp.GREATER, "2021-01-01 00:00:00", "1970-01-01 00:08:00");
			put.setObject(0, 0);
			put.setObject(1, "name0_new");
			put.setObject(2, "2021-01-01 00:00:00");
			put.setObject(3, "address0_new");
			client.checkAndPut(put);
			put = new CheckAndPut(schema, "modify_time", CheckCompareOp.GREATER, "2022-01-01 00:00:00", "1970-01-01 00:08:00");
			put.setObject(0, 0);
	...
	public void testCheckAndPutBatch006() throws Exception {
		...
		try (Connection conn = buildConnection(); HoloClient client = new HoloClient(config)) {
			...
			*/
			CheckAndPut cput = new CheckAndPut(schema, "modify_time", CheckCompareOp.GREATER, "2020-01-01 00:00:00", "1970-01-01 00:08:00");
			cput.setObject(0, 0);
			cput.getRecord().setType(Put.MutationType.DELETE);
			client.checkAndPut(cput);
			cput = new CheckAndPut(schema, "modify_time", CheckCompareOp.GREATER, "2020-01-01 00:00:00", "1970-01-01 00:08:00");
			cput.setObject(0, 1);
			cput.getRecord().setType(Put.MutationType.DELETE);
			client.checkAndPut(cput);
			client.flush();
	...
	/**
	 * DELETE, new value check old value
	 * Method: checkAndPut(CheckAndPut put).
	 */
	@Test
	public void testCheckAndPutBatch007() throws Exception {
		...
		try (Connection conn = buildConnection(); HoloClient client = new HoloClient(config)) {
			...
			try {
				...
				Assert.assertTrue(e.getMessage().contains("checkColumn modify_time should be set not null when not set checkValue and mutationType is delete."));
			}
	...
	public void testCheckAndPutBatch008() throws Exception {
		...
		try (Connection conn = buildConnection(); HoloClient client = new HoloClient(config)) {
			...
			// delete from "public"."holo_client_check_and_put_008" where (id=$1 and '2020-01-01 00:00:00'::timestamptz > coalesce(modify_time, '1970-01-01 00:08:00'::timestamptz))
			put = new CheckAndPut(schema, "modify_time", CheckCompareOp.GREATER, "2020-01-01 00:00:00", "1970-01-01 00:08:00");
			put.setObject(0, 0);
			put.getRecord().setType(Put.MutationType.DELETE);
			client.checkAndPut(put);
			...
			put.getRecord().setType(Put.MutationType.DELETE);
			client.checkAndPut(put);
			// delete from "public"."holo_client_check_and_put_008" where (id=$1 and '2021-01-01 00:00:00'::timestamptz > coalesce(modify_time, '1970-01-01 00:08:00'::timestamptz))
			put = new CheckAndPut(schema, "modify_time", CheckCompareOp.GREATER, "2021-01-01 00:00:00", "1970-01-01 00:08:00");
			put.setObject(0, 0);
			put.getRecord().setType(Put.MutationType.DELETE);
			client.checkAndPut(put);
	...
	private void initData(HoloClient client, TableSchema schema, Object initValue) throws HoloClientException {
		Put put = new Put(schema);
		put.setObject("id", 0);
	...
	public void testGreaterOrEqual001() throws Exception {
		...
		try (Connection conn = buildConnection(); HoloClient client = new HoloClient(config)) {
			...
			String dropSql = "drop table if exists " + tableName;
			String createSql = "create table " + tableName + "(id int not null,name text, age int ,primary key(id))";
			...
			try {
				initData(client, schema);
				{
					...
					put.setObject("id", 6);
					put.setObject("name", "name1");
					put.setObject("age", null); // coalesce(null as 0), new value is null, will not update
					client.checkAndPut("age", CheckCompareOp.GREATER_OR_EQUAL, null, 0, put);
					client.flush();
					...
					record = client.get(Get.newBuilder(schema).setPrimaryKey("id", 1).build()).get();
					Assert.assertEquals("name1", record.getObject("name"));
					Assert.assertEquals(0, record.getObject("age"));
					record = client.get(Get.newBuilder(schema).setPrimaryKey("id", 2).build()).get();
					Assert.assertEquals("name1", record.getObject("name"));
					Assert.assertEquals(1, record.getObject("age"));
					record = client.get(Get.newBuilder(schema).setPrimaryKey("id", 3).build()).get();
					Assert.assertEquals("name0", record.getObject("name"));
					Assert.assertEquals(null, record.getObject("age"));
					record = client.get(Get.newBuilder(schema).setPrimaryKey("id", 4).build()).get();
					Assert.assertEquals("name1", record.getObject("name"));
					Assert.assertEquals(0, record.getObject("age"));
					record = client.get(Get.newBuilder(schema).setPrimaryKey("id", 5).build()).get();
					Assert.assertEquals("name1", record.getObject("name"));
					Assert.assertEquals(1, record.getObject("age"));
					record = client.get(Get.newBuilder(schema).setPrimaryKey("id", 6).build()).get();
					Assert.assertEquals("name0", record.getObject("name"));
					Assert.assertEquals(null, record.getObject("age"));
				}
			} finally {
		...
		try (Connection conn = buildConnection(); HoloClient client = new HoloClient(config)) {
			...
			try {
				initData(client, schema);
				{
					...
					put.setObject("id", 5);
					put.setObject("name", "name1");
					put.setObject("age", 1); // coalesce(null as 0), same as greater, will not update
					client.checkAndPut("age", CheckCompareOp.LESS_OR_EQUAL, null, 0, put);
					put = new Put(schema);
					put.setObject("id", 6);
					put.setObject("name", "name1");
		...
		try (Connection conn = buildConnection(); HoloClient client = new HoloClient(config)) {
			...
			try {
				initData(client, schema);
				{
					...
					put.getRecord().setType(Put.MutationType.DELETE);
					put.setObject("id", 5);
					put.setObject("age", 1); // coalesce(null as 0), same as greater, will not delete
					client.checkAndPut("age", CheckCompareOp.LESS_OR_EQUAL, null, 0, put);
					client.flush();
	...
	private Object[] makeTestDataForDataTypes(String name) {
		switch (name) {
			...
			case "decimal":
				// numeric(6,5)
				return new Object[]{new BigDecimal("-1.00000"), new BigDecimal("0.00000"), new BigDecimal("1.00000")};
			case "bool":
	...
	public void testGreaterOrEqualDataTypes001(DataTypeTestUtil.TypeCaseData typeCaseData) throws Exception {
		if (properties == null) {
			return;
		}
		...
		try (Connection conn = buildConnection(); HoloClient client = new HoloClient(config)) {
			...
			String dropSql = "drop table if exists " + tableName;
			String createSql = "create table " + tableName + "(id int not null,name text, age " + typeCaseData.getColumnType() + " ,primary key(id))";
			...
			try {
				initData(client, schema, testData[1]);
				{
					...
					client.checkAndPut("age", CheckCompareOp.GREATER_OR_EQUAL, null, testData[1], put);
					put = new Put(schema);
					put.setObject("id", 1);
					put.setObject("name", "name1");
					...
					put.setObject("id", 3);
					put.setObject("name", "name1");
					put.setObject("age", testData[0]); // coalesce(null as 0), same as less, will not update
					client.checkAndPut("age", CheckCompareOp.GREATER_OR_EQUAL, null, testData[1], put);
					put = new Put(schema);
					put.setObject("id", 4);
					put.setObject("name", "name1");
	 ...
	 */
	@Test(dataProvider = "typeCaseData")
	public void testGreaterOrEqualDataTypes002(DataTypeTestUtil.TypeCaseData typeCaseData) throws Exception {
		if (properties == null) {
			return;
		}
		String typeName = typeCaseData.getName();
		...
		try (Connection conn = buildConnection(); HoloClient client = new HoloClient(config)) {
			...
			try {
				initData(client, schema, testData[1]);
				{
					...
					put.getRecord().setType(Put.MutationType.DELETE);
					put.setObject("id", 3);
					put.setObject("age", testData[0]); // coalesce(null as 0), same as less, will not delete
					client.checkAndPut("age", CheckCompareOp.GREATER_OR_EQUAL, null, testData[1], put);
					put = new Put(schema);
					put.getRecord().setType(Put.MutationType.DELETE);
					put.setObject("id", 4);
