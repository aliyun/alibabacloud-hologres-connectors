...
/** 特殊类型的支持测试. 测试库需要已经安装了postgis和roaringbitmap. */
public class HoloClientTypesTest extends HoloClientTestBase {

    @DataProvider(name = "typeCaseData")
    public Object[][] createData() {
        // fixed fe只测试fixed plan支持的类型
        Object[][] ret = new Object[FIXED_PLAN_TYPE_DATA.length + ALL_TYPE_DATA.length][];
        for (int i = 0; i < FIXED_PLAN_TYPE_DATA.length; ++i) {
            ret[i] = new Object[] {FIXED_PLAN_TYPE_DATA[i], true};
        }
        int offset = FIXED_PLAN_TYPE_DATA.length;
        for (int i = 0; i < ALL_TYPE_DATA.length; ++i) {
            ret[offset + i] = new Object[] {ALL_TYPE_DATA[i], false};
        }
        return ret;
    }
    ...
    public Object[][] createDataForGet() {
        ...
        Object[][] ret =
                new Object
                        [FIXED_PLAN_TYPE_DATA_WITH_RECORD.length + ALL_TYPE_DATA_WITH_RECORD.length]
                        [];
        for (int i = 0; i < FIXED_PLAN_TYPE_DATA_WITH_RECORD.length; ++i) {
            ret[i] = new Object[] {FIXED_PLAN_TYPE_DATA_WITH_RECORD[i], true};
        }
        int offset = FIXED_PLAN_TYPE_DATA_WITH_RECORD.length;
        for (int i = 0; i < ALL_TYPE_DATA_WITH_RECORD.length; ++i) {
            ret[offset + i] = new Object[] {ALL_TYPE_DATA_WITH_RECORD[i], false};
        }
        return ret;
    }
    ...
    public void testGIS() throws Exception {
        ...
        try (Connection conn = buildConnection();
            ...
            String createSql =
                    "create table " + tableName + "(id int PRIMARY KEY, b geography(POINT,4326))";
    ...
    public void testALLTypeInsert(DataTypeTestUtil.TypeCaseData typeCaseData, boolean useFixedFe)
            throws Exception {
        if (properties == null) {
            return;
        }
    ...
    /** allType. times of get > 5, to test statement name is saved Method: get(Get get). */
    @Test(dataProvider = "typeCaseDataWithRecord")
    public void testALLTypeGet(
            DataTypeTestUtil.TypeCaseDataWithRecord typeCaseData, boolean useFixedFe)
            throws Exception {
        if (properties == null) {
            return;
        }
        ...
        // TODO: fixedfe float4[] and float8[] result not equal with fe
        // expected value is <3.4028235E38> but was <3.40282E38>.
        // expected value is <1.7976931348623157E308> but was <Infinity>.
        if ((typeName.equals("_float4") || typeName.equals("_float8")) && useFixedFe) {
            return;
        }
        config.setUseFixedFe(useFixedFe);
        config.setUseLegacyGetHandler(true);
        ...
        try (Connection conn = buildConnection();
            ...
            try {
                ...
                for (int i = 0; i < 10; i++) {
                    Record r =
                            client.get(Get.newBuilder(schema).setPrimaryKey("pk", i).build()).get();
                    if (i == nullPkId) {
    ...
    public void testALLTypeInsertDirtyDataError(DataTypeTestUtil.TypeCaseData typeCaseData)
            throws Exception {
        if (properties == null) {
            return;
        }
        ...
        HoloConfig config = buildConfig();
        // fixed fe not support jsonb and roaringbitmap now
        if ((typeName.equals("roaringbitmap") || typeName.equals("jsonb"))
                && config.isUseFixedFe()) {
            return;
        }
        config.setAppName("testALLTypeInsert");
        ...
        try (Connection conn = buildConnection();
            ...
            execute(
                    conn,
                    new String[] {
                        "CREATE EXTENSION if not exists roaringbitmap", dropSql, createSql
                    });
            try {
                TableSchema schema = client.getTableSchema(tableName, true);
