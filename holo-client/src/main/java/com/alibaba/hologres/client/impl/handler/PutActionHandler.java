...
public class PutActionHandler extends ActionHandler<PutAction> {
    ...
    public PutActionHandler(ConnectionHolder connectionHolder, HoloConfig config) {
        ...
        this.config = config;
        this.builder =
                config.isUseLegacyPutHandler()
                        ? new UpsertStatementBuilder(config)
                        : new UnnestUpsertStatementBuilder(config);
    ...
    public void handle(PutAction action) {
        ...
        OnConflictAction onConflictAction = action.getOnConflictAction();
        HoloClientException exception = null;
        try {
            doHandlePutAction(recordList, onConflictAction);
            ...
            switch (strategy) {
                case TRY_ONE_BY_ONE:
                    ...
                    if (e.getCode() != ExceptionCode.TABLE_NOT_FOUND) {
                        ...
                        HoloClientWithDetailsException fails =
                                new HoloClientWithDetailsException(e);
                        for (Record record : recordList) {
                            try {
                                single.add(record);
                                doHandlePutAction(single, onConflictAction);
                                markRecordPutSuccess(record);
                                ...
                                exception =
                                        new HoloClientException(
                                                ExceptionCode.INTERNAL_ERROR, "", subE);
            ...
            if (useDefaultStrategy) {
                ...
                if (exception == null) {
                    HoloClientWithDetailsException localPutException =
                            new HoloClientWithDetailsException(e);
                    ...
                    try {
                        ...
                        for (PreparedStatementWithBatchInfo ps : psArray) {
                            ...
                            if (config.isEnableLogSlowQuery()) {
                                if (slowQuery == null
                                        || (cost > config.getLogSlowQueryThresholdMs()
                                                && cost > slowQuery.f1)) {
                                    slowQuery =
                                            new Tuple4<>(
                                                    startTime,
                                                    cost,
                                                    ps.l.getUpdateCount(),
                                                    ConnectionUtil.getQueryIdFromNotice(
                                                            ps.l.getWarnings()));
                                }
                                if (startTime - lastLogSlowQueryIdTime
                                        > config.getLogSlowQueryIntervalSeconds() * 1000L) {
                                    lastLogSlowQueryIdTime = startTime;
                                    LOGGER.warn(
                                            "The slowest insert query in the past {}s was started at [{}], cost [{}] ms, and affected [{}] rows{}.",
                        ...
                        // registry.histogram(Metrics.METRICS_WRITE_LATENCY +
                        // tableName).update(endTime - startTime);
                        registry.meter(Metrics.METRICS_WRITE_QPS).mark();
                        registry.meter(Metrics.METRICS_WRITE_BPS).mark(bytes);
                        if (batchCount > 0) {
                            registry.histogram(Metrics.METRICS_WRITE_SQL_PER_BATCH)
                                    .update(batchCount);
                        }
                        registry.histogram(Metrics.METRICS_WRITE_LATENCY)
