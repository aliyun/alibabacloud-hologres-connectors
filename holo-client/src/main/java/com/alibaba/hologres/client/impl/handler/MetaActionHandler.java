                    ...
                    .complete(
                            (TableSchema)
                                    connectionHolder.retryExecuteWithVersion(
                                            (connWithVersion) -> {
                                                if (config.isRefreshMetaBeforeGetTableSchema()
                                                        && config.getRefreshMetaTimeout() > 0) {
                                                    String fullTableName =
                                                            action.getTableName().getFullName();
                                                    ConnectionUtil.CheckMetaResult result =
                                                            ConnectionUtil.checkMeta(
                                                                    connWithVersion.getConn(),
                                                                    connWithVersion.getVersion(),
                                                                    fullTableName,
                                                                    config.getRefreshMetaTimeout());
                                                    if (!result.isUpdated()) {
                                                        if (result.getMsg()
                                                                .contains(
                                                                        "table is lock by other query which request a AccessExclusiveLock")) {
                                                            throw new SQLException(
                                                                    "mismatches the version of the table ["
                                                                            + fullTableName
                                                                            + "]:"
                                                                            + result
                                                                                    .getMsg()); // HoloClientException会识别这种特殊的异常message
                                                        } else {
                                                            throw new SQLException(
                                                                    "truncate table ["
                                                                            + fullTableName
                                                                            + "], but replay not finished yet:"
                                                                            + result
                                                                                    .getMsg()); // HoloClientException会识别这种特殊的异常message
                                                        }
                                                    }
                                                }
                                                return ConnectionUtil.getTableSchema(
                                                        connWithVersion.getConn(),
                                                        action.getTableName());
                                            }));
