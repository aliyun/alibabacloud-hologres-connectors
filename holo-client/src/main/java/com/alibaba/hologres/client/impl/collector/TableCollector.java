...
/**
 * PutAction收集器（表级别）. 每一个HoloClient对应一个ActionClient ActionCollector - TableCollector
...
public class TableCollector {
    ...
    public long getByteSize() {
        return Arrays.stream(pairArray)
                .collect(Collectors.summingLong(TableShardCollector::getByteSize));
    }
    ...
    public void append(Record record) throws HoloClientException {
        ...
        if (recordSampleInterval > 0 && nano - lastSampleTime > recordSampleInterval) {
            ...
            try {
                ...
                LOG.info(
                        "sample data: table name={}, record={}, attachObj={}",
                        record.getSchema().getTableNameObj(),
                        record,
                        attachmentObj);
            } catch (Exception e) {
    ...
    public boolean flush(boolean force, boolean async, AtomicInteger uncommittedActionCount)
