    ...
    public static class Builder {
            ...
            if (index == null) {
                throw new InvalidParameterException(
                        String.format(
                                ...
                                schema.getTableName(), columnName));
            }
            if (value == null) {
                throw new InvalidParameterException(
                        String.format(
                                "Scan can not set null value for addEqualFilter. table %s ,column %s.",
                                schema.getTableName(), columnName));
            }
            if (filterList == null) {
        ...
        public Builder addRangeFilter(String columnName, Object start, Object end) {
            ...
            if (start == null && end == null) {
                throw new InvalidParameterException(
                        String.format(
                                "Scan can not set start and end both null. table %s ,column %s.",
                                schema.getTableName(), columnName));
            }
            if (filterList == null) {
                filterList = new ArrayList<>();
        ...
        public Builder setFetchSize(int fetchSize) {
            if (fetchSize <= 0) {
                throw new InvalidParameterException(
                        String.format("FetchSize must > 0 . Table %s.", schema.getTableName()));
            }
            this.fetchSize = fetchSize;
            return this;
        }
        ...
        public Builder setTimeout(int timeout) {
            if (timeout <= 0) {
                throw new InvalidParameterException(
                        String.format("Timeout must > 0 . Table %s.", schema.getTableName()));
            }
            this.timeout = timeout;
            return this;
        }
