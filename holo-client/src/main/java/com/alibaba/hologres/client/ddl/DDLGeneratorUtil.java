...
public class DDLGeneratorUtil {
    ...
    public static String generateCreateTableStatement(TableSchema table) {
        ...
        if (table.getNotExist()) {
            statement
                    ...
                    .append(SPACE)
                    .append(String.format("%s.%s", table.getSchemaName(), table.getTableName()));
        } else {
            statement
                    .append(CREATE)
                    .append(SPACE)
    ...
    public static String generatesCommentStatement(TableSchema table) {
        ...
        if (CommonUtil.isNotEmpty(table.getComment())) {
            statement.append(
                    genCommentStatement(
                            String.format("%s.%s", table.getSchemaName(), table.getTableName()),
                            null,
                            table.getComment()));
        }
    ...
    private static String generateTableColumnsStatement(Column[] columns) {
        ...
        for (int i = 0; i < columns.length; i++) {
            ...
            if (column.isArrayType() != null && column.isArrayType()) {
    ...
    private static String generateTableCallStatement(TableSchema table) {
        ...
        if (CommonUtil.isNotEmpty(table.getOrientation())) {
            statement.append(
                    genCallPrecedureStatement(tableName, "orientation", table.getOrientation()));
        }
        if (CommonUtil.isNotEmpty(table.getClusteringKey())) {
            statement.append(
                    genCallPrecedureStatement(
                            tableName,
                            "clustering_key",
                            CommonUtil.join(table.getClusteringKey(), ",")));
        }
        if (CommonUtil.isNotEmpty(table.getSegmentKey())) {
            statement.append(
                    genCallPrecedureStatement(
                            tableName, "segment_key", CommonUtil.join(table.getSegmentKey(), ",")));
        }
        if (CommonUtil.isNotEmpty(table.getBitmapIndexKey())) {
            statement.append(
                    genCallPrecedureStatement(
                            tableName,
                            "bitmap_columns",
                            CommonUtil.join(table.getBitmapIndexKey(), ",")));
        }
        if (CommonUtil.isNotEmpty(table.getDictionaryEncoding())) {
            statement.append(
                    genCallPrecedureStatement(
                            ...
                            CommonUtil.join(table.getDictionaryEncoding(), ",")));
        }
        if (table.getLifecycle() != null) {
            statement.append(
                    genCallPrecedureStatement(
                            tableName, "time_to_live_in_seconds", table.getLifecycle().toString()));
        }
        return statement.toString();
    }
