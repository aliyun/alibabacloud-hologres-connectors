...
public class DDLGenerator {
    ...
    private static String validate(TableSchema table) {
        ...
        if (CommonUtil.isNotEmpty(table.getPartitionInfo())) {
            errorMsg =
                    checkIndexInColumns(
                            new String[] {table.getPartitionInfo()},
                            table.getColumnSchema(),
                            "partitionKey",
                            false,
                            table.getSensitive());
    ...
    private static String checkIndexInColumns(
            String[] indexKeys,
            Column[] columns,
            String keyName,
            boolean allowedNull,
            ...
            if (!isSensitive) {
                ...
                if (!allowedNull && !columnMap.get(key).getAllowNull().equals(allowedNull)) {
                    return String.format(
                            "%s : %s shouldn't be nullable", keyName, columnMap.get(key).getName());
                }
            } else {
