...
public class BinlogShardGroupReader implements Closeable {
    ...
    public BinlogShardGroupReader(
            HoloConfig config,
            Subscribe subscribe,
            Map<Integer, Committer> committerMap,
                ...
                if (committerMap != null) {
                    ...
                    if (committer == null) {
                        throw new HoloClientException(
                                ...
                                "reader for shard " + r.getShardId() + " is not exists!");
                    }
    ...
    @Deprecated
    public void commit(long timeoutMs)
            throws HoloClientException, TimeoutException, InterruptedException {
        if (committerMap == null) {
            throw new HoloClientException(
                    ExceptionCode.INTERNAL_ERROR, "could not commit without committer");
        }
            ...
            if (currentMs < targetMs) {
                try {
                    ...
                    if (cause instanceof HoloClientException) {
                        ...
                        throw new HoloClientException(
                                ExceptionCode.INTERNAL_ERROR, "commit fail", cause);
                    }
    ...
    public CompletableFuture<Void> commitFlushedLsn(
            Committer committer, int shardId, long lsn, long timeoutMs)
            throws TimeoutException, InterruptedException {
        LOGGER.info(
                "begin commit {} shardId {} flushedLsn to {}",
                subscribe.getTableName(),
                shardId,
                lsn);
        return committer
                .commit(lsn, timeoutMs)
                .thenRun(
                        () -> {
                            LOGGER.info(
                                    "done commit {} shardId {} flushedLsn to {}",
                                    subscribe.getTableName(),
                                    shardId,
                                    lsn);
                        });
    ...
    public void commitFlushedLsn(int shardId, long lsn, long timeoutMs)
            throws HoloClientException, TimeoutException, InterruptedException {
        ...
        if (committer != null) {
            ...
            throw new HoloClientException(
                    ExceptionCode.INVALID_REQUEST, "unknown shard " + shardId);
        }
    }
