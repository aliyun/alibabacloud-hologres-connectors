...
public class Cache<K, V> {
    ...
    /**
     * 获取符合所有下述条件的key列表. 1 未过期 2 accessed为ture 3 剩余存活时间少于remainLifeTime
     *
    ...
    public Set<K> filterExpireKeys(long remainLifeTime) {
        ...
        return cache.entrySet().stream()
                .filter(
                        entry ->
                                !entry.getValue().isExpire(current)
        ...
        return cache.entrySet().stream()
                .filter(
                        entry ->
                                entry.getValue().isExpire(current)
                                        || (entry.getValue().isAccessed()
                                                && ttl - entry.getValue().getAge(current)
                                                        < remainLifeTime))
                .map(
                        entry ->
                                new Tuple<>(
     ...
     * @param mode MODE_LOCAL_THEN_REMOTE 先查缓存，没有命中（不存在或过期）则调用builder生成value MODE_NO_CACHE
     *     调用builder生成value MODE_ONLY_CACHE 查缓存，存在且没有过期返回value，否则返回null
     * @return 结果
     * @throws SQLException builder失败抛的异常
     */
    public V get(K key, FunctionWithSQLException<K, V> builder, final int mode)
            throws SQLException {
        final FunctionWithSQLException<K, V> internalBuilder =
                builder == null ? this.builder : builder;
        Item item = null;
