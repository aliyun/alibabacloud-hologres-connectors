...
public class Record implements Serializable {
    ...
    /**
     * 只在put场景下使用，存储这个Record对应的所有put的future. 例：
    ...
    private long getObjByteSize(int index, Object obj) {
        ...
        switch (column.getType()) {
            ...
            case Types.ARRAY:
                ...
                break;
            default:
                if ("json".equalsIgnoreCase(column.getTypeName())
                        || "jsonb".equalsIgnoreCase(column.getTypeName())) { // json, jsonb 等类型
                    ret = String.valueOf(obj).length();
                    break;
                }
    ...
    /**
     * a.merge(b). a的值被b"覆盖"（只有b
     * set过值会覆盖到a上），a.attachmentList=a.attachmentList+b.attachmentList，b盖在a上面.
     *
     * @param record
     */
