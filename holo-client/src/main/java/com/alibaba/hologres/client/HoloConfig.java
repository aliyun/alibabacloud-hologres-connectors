...
public class HoloConfig implements Serializable {
    ...
    public static final OnConflictAction DEFAULT_ON_CONFLICT_ACTION =
    ...
    // ----------------------------write conf--------------------------------------------
    /**
     * 在AsyncCommit为true，调用put方法时，当记录数>=writeBatchSize 或 总记录字节数数>=writeBatchByteSize. 调用flush进行批量提交.
     * 默认为512 @HasGetter @HasSetter
     */
    int writeBatchSize = DEFAULT_BATCH_SIZE;

    /**
     * 在AsyncCommit为true，调用put方法时，当记录数>=writeBatchSize 或 总记录字节数数>=writeBatchByteSize.
     * 调用flush进行List批量提交. 默认为2MB @HasGetter @HasSetter
     */
    long writeBatchByteSize = DEFAULT_BATCH_BYTE_SIZE;

    /** 所有表攒批总共的最大batchSize 调用flush进行List批量提交. 默认为20MB @HasGetter @HasSetter */
    long writeBatchTotalByteSize = DEFAULT_BATCH_BYTE_SIZE * 10;

    /**
     * 当INSERT目标表为有主键的表时采用不同策略. INSERT_OR_IGNORE 当主键冲突时，不写入 INSERT_OR_UPDATE 当主键冲突时，更新相应列
     * INSERT_OR_REPLACE当主键冲突时，更新所有列 @HasGetter @HasSetter
     */
    OnConflictAction onConflictAction = DEFAULT_ON_CONFLICT_ACTION;

    /** 启用后，当put分区表时，若分区不存在将自动创建，默认false. @HasGetter @HasSetter */
    boolean dynamicPartition = false;

    /**
     * 在AsyncCommit为true，当记录数>=writeBatchSize 或 总记录字节数数>=writeBatchByteSize
     * 或距离上次flush超过writeMaxIntervalMs毫秒 调用flush进行提交 * 默认为100MB. @HasGetter @HasSetter
     */
    long writeMaxIntervalMs = 10000L;

    /** 当INSERT失败采取的策略. TRY_ONE_BY_NE NONE @HasGetter @HasSetter */
    WriteFailStrategy writeFailStrategy = WriteFailStrategy.TRY_ONE_BY_ONE;

    /** put操作的并发数. @HasGetter @HasSetter */
    int writeThreadSize = 1;

    /**
     * 当将Number写入Date/timestamp/timestamptz列时，将number视作距离1970-01-01 00:00:00
     * +00:00的毫秒数. @HasGetter @HasSetter
     */
    ...
    /** 启用时，not null且未在表上设置default的字段传入null时，将转为默认值. String @HasGetter @HasSetter */
    boolean enableDefaultForNotNullColumn = true;

    /**
     * psql中对于 insert/update/delete语句的执行默认是会返回affect的行数，比如向空表insert into table values (?,?) 会返回
     * INSERT 0 1，这种行为在某些场景下可能增加开销； 而holoclient中目前不需要收集affect的行数，考虑导性能，默认关闭affect
     * rows. @HasGetter @HasSetter
     */
    boolean enableAffectedRows = false;

    /** defaultTimestamp. String @HasGetter @HasSetter */
    String defaultTimestampText = null;

    /** 写入shard数重计算的间隔. long @HasGetter @HasSetter */
    long writerShardCountResizeIntervalMs = 30000L;

    /** 开启将text列value中的\u0000替换为"". boolean @HasGetter @HasSetter */
    boolean removeU0000InTextColumnValue = true;

    /** 全局flush的时间间隔. boolean @HasGetter @HasSetter */
    long forceFlushInterval = -1L;

    /** 最大shard数. boolean @HasGetter @HasSetter */
    int maxShardCount = -1;

    /** CopyIn时，InputStream和网络OutputStream之间的交互数据的buffer大小. int @HasGetter @HasSetter */
    int copyInBufferSize = 65536;

    /** 多久打印一次写入数据采样. int @HasGetter @HasSetter */
    long recordSampleInterval = -1L;

    /**
     * 强制使用老方式做put请求 insert into xxx values (),() on conflict. 不强制的话，对于高版本holo会自动采用insert into xxx
     * select unnest() on conflict的方式写入 boolean @HasGetter @HasSetter
     */
    boolean useLegacyPutHandler = false;

    /** 写入时一条sql的最大行数，仅unnest模式生效. [TODO] 仍在测试中. int @HasGetter @HasSetter */
    int maxRowsPerSql = Integer.MAX_VALUE;

    /** 写入时一条sql的最大字节数，仅unnest模式生效. [TODO] 仍在测试中. long @HasGetter @HasSetter */
    ...
    /** 激进模式,写入时发现连接空闲,即使未攒够批也立刻提交. boolean @HasGetter @HasSetter */
    boolean enableAggressive = false;

    /** 写入和点查的时间不符合预期时,在日志中打印耗时较久的query执行信息,如果有query id也一起打印. */
    boolean enableLogSlowQuery = true;

    /** 写入和点查的耗时大于此参数的进行收集. */
    long logSlowQueryThresholdMs = 2000L;
    /**
     * 写入和点查的时间不符合预期时, 在日志中打印query执行信息的周期, 默认一分钟一次, 打印上一分钟耗时超过logSlowQueryThresholdMs的最慢的query.
    ...
    /** get请求缓冲池大小，默认256. @HasGetter @HasSetter */
    int readBatchQueueSize = DEFAULT_READ_QUEUE;

    /** get请求缓冲池拿数据的超时时间，默认10000,即10s. @HasGetter @HasSetter */
    long readBatchQueueOfferTimeoutMs = 10000L;
    /** get操作的并发数. @HasGetter @HasSetter */
    int readThreadSize = 1;

    /**
     * get操作的超时时间. 默认值0表示不超时，会在两处位置生效： 1.get操作从用户开始执行到client准备提交到holo的等待时间，如果这个阶段超时不会重试直接抛出异常. 2.get
     * sql的执行超时，即statement query timeout，最小1s. @HasGetter @HasSetter
     */
    int readTimeoutMilliseconds = 0;

    /** get请求重试次数, 与默认的retryCount分别设置，默认1不会retry. @HasGetter @HasSetter */
    int readRetryCount = 1;

    /**
     * 强制使用老方式做get请求 select * from table where (key1=? and key2=?) or (key1=? and key2=?)
     * 不强制的话，对于高版本holo会自动采用select * from table where (key1,key2) in (select unnest(?),
     * unnest(?))的方式点查 boolean @HasGetter @HasSetter
     */
    boolean useLegacyGetHandler = false;

    // --------------------------scan conf-------------------------------------------------
    /** scan每次fetch的大小. @HasGetter @HasSetter */
    int scanFetchSize = 2000;
    /** scan的超时时间. @HasGetter @HasSetter */
    int scanTimeoutSeconds = 60;

    // --------------------------binlog read conf-------------------------------------------------
    /** 一次读取 binlogReadBatchSize 条 Binlog 数据. @HasGetter @HasSetter */
    int binlogReadBatchSize = 1024;

    /**
     * binlog read的超时时间. 默认值-1表示不超时
     * 超时时间大于0时，必须设置binlogHeartBeatIntervalMs参数，要求binlogHeartBeatIntervalMs <
     * binlogReadTimeoutMs @HasGetter @HasSetter
     */
    long binlogReadTimeoutMs = -1;

    /**
     * binlogRead 发送BinlogHeartBeatRecord的间隔. -1表示不发送
     * 当binlog没有新数据，每binlogHeartBeatIntervalMs会下发一条BinlogHeartBeatRecord，record的timestamp表示截止到这个时间的数据都已经消费完了. @HasGetter @HasSetter
     */
    long binlogHeartBeatIntervalMs = -1L;

    /** 是否需要忽略Delete类型的Binlog. @HasGetter @HasSetter */
    boolean binlogIgnoreDelete = false;

    /** 是否需要忽略BeforeUpdate类型的Binlog. @HasGetter @HasSetter */
    boolean binlogIgnoreBeforeUpdate = false;

    /** 分区子表消费模式. @HasGetter @HasSetter */
    BinlogPartitionSubscribeMode binlogPartitionSubscribeMode =
            BinlogPartitionSubscribeMode.DISABLE;

    /** 对于历史分区，会保留一段时间，防止上游数据有延迟. @HasGetter @HasSetter */
    int binlogPartitionLatenessTimeoutSecond = 60;

    // ---------------------------conn conf------------------------------------------
    /** 请求重试次数，默认3. 这个名字应该叫做maxTryCount，而不是retryCount，设为1其实是不会retry的 @HasGetter @HasSetter */
    int retryCount = 3;

    /** 每次重试等待时间为 当前重试次数*retrySleepMs + retrySleepInitMs. @HasGetter @HasSetter */
    long retrySleepStepMs = 10000L;

    /** 每次重试等待时间为 当前重试次数*retrySleepMs + retrySleepInitMs. @HasGetter @HasSetter */
    long retrySleepInitMs = 1000L;

    /** 每个get和put的后台连接在空闲超过connectionMaxIdleMs后将被释放(再次使用时会自动重新连接). @HasGetter @HasSetter */
    long connectionMaxIdleMs = 60000L;

    /** 每个get和put的后台连接最大存活时间,超过之后将被释放(再次使用时会自动重新连接). 尽可能避免服务端可能存在的内存泄漏等问题. @HasGetter @HasSetter */
    long connectionMaxAliveMs = 60 * 60 * 1000L;

    /** meta信息缓存时间(ms). @HasGetter @HasSetter */
    long metaCacheTTL = 60000L;

    /** meta缓存剩余时间低于 metaCacheTTL/metaAutoRefreshFactor 将被自动刷新. @HasGetter @HasSetter */
    int metaAutoRefreshFactor = 4;

    /** 执行hg_internal_refresh_meta的默认超时时间(单位为秒). 若<=0则不执行 @HasGetter @HasSetter */
    int refreshMetaTimeout = 10;

    /** connection建立后是否执行hg_internal_refresh_meta. @HasGetter @HasSetter */
    boolean refreshMetaAfterConnectionCreated = false;

    /** 获取tableSchema前是否执行hg_internal_refresh_meta. @HasGetter @HasSetter */
    boolean refreshMetaBeforeGetTableSchema = true;

    /**
     * 是否直连holo frontend. 开启此参数会尝试绕过vip直接连接fe, 用于用户的vip带宽不能满足需求的场景.
     * 如果网络环境不支持直连，会回退到通过vip连接. @HasGetter @HasSetter
     */
    boolean enableDirectConnection = false;

    /**
     * 是否启用SSL（Secure Sockets Layer）传输加密. 详见文档 <a
     * href="https://help.aliyun.com/zh/hologres/user-guide/ssl-encrypted-transmission">...</a> @HasGetter @HasSetter
     */
    SSLMode sslMode = SSLMode.DISABLE;

    /** SSL证书路径， 当sslMode设置位VERIFY_CA或者VERIFY_FULL时需要. @HasGetter @HasSetter */
    String sslRootCertLocation = null;

    // ------------------------endpoint conf--------------------------------------------
    /** 顾名思义，jdbcUrl. @HasGetter @HasSetter */
    String jdbcUrl;

    /** jdbcUrl，必填. @HasGetter @HasSetter */
    String username;

    /** jdbcUrl，必填. @HasGetter @HasSetter */
    String password;

    /** 是否使用AKv4方式编码username和password. @HasGetter @HasSetter */
    boolean useAKv4 = true;

    /** 使用AKv4方式编码username和password时指定实例地域. @HasGetter @HasSetter */
    String region;

    /** 是否使用fixed fe模式进行数据写入和点查. @HasGetter @HasSetter */
    boolean useFixedFe = false;

    /** 使用fixed fe进行数据写入和点查时， 其他action使用的连接数. @HasGetter @HasSetter */
    ...
    public WriteMode getWriteMode() {
        return WriteMode.fromOnConflictAction(getOnConflictAction());
    }

    public void setOnConflictAction(OnConflictAction onConflictAction) {
        this.onConflictAction = onConflictAction;
    }

    @Deprecated
    public void setWriteMode(WriteMode writeMode) {
        setOnConflictAction(OnConflictAction.fromWriteMode(writeMode));
    }
    ...
    public boolean isUseAKv4() {
        return useAKv4;
    }

    public void setUseAKv4(boolean useAKv4) {
        this.useAKv4 = useAKv4;
    }
    ...
    public void setInputNumberAsEpochMsForDatetimeColumn(
    ...
    public void setInputStringAsEpochMsForDatetimeColumn(
    ...
    public void setBinlogPartitionSubscribeMode(
            BinlogPartitionSubscribeMode binlogPartitionSubscribeMode) {
    ...
    public boolean isUseLegacyGetHandler() {
        return useLegacyGetHandler;
    }

    public void setUseLegacyGetHandler(boolean useLegacyGetHandler) {
        this.useLegacyGetHandler = useLegacyGetHandler;
    }
    ...
    public boolean isEnableLogSlowQuery() {
        return enableLogSlowQuery;
    }

    public void setEnableLogSlowQuery(boolean enableLogSlowQuery) {
        this.enableLogSlowQuery = enableLogSlowQuery;
    }

    public long getLogSlowQueryThresholdMs() {
        return logSlowQueryThresholdMs;
    }

    public void setLogSlowQueryThresholdMs(long logSlowQueryThresholdMs) {
        this.logSlowQueryThresholdMs = logSlowQueryThresholdMs;
    }

    public long getLogSlowQueryIntervalSeconds() {
        return logSlowQueryIntervalSeconds;
    }
    ...
    public long getReadBatchQueueOfferTimeoutMs() {
        return readBatchQueueOfferTimeoutMs;
    }

    public void setReadBatchQueueOfferTimeoutMs(long readBatchQueueOfferTimeoutMs) {
        this.readBatchQueueOfferTimeoutMs = readBatchQueueOfferTimeoutMs;
    }
}
