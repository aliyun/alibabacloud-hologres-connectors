        ...
        try {
            ...
            if (handler == null) {
                throw new HoloClientException(
                        ...
                        "Unknown action:" + action.getClass().getName());
            }
    ...
    public void run() {
        ...
        while (started.get()) {
            try {
                ...
                if (System.currentTimeMillis() - connectionHolder.getLastActiveTs()
                        > config.getConnectionMaxIdleMs()) {
                    connectionHolder.close("close connection due to max idle time exceeded.");
                }
                if (System.currentTimeMillis() - connectionHolder.getConnCreateTs()
                        > connectionMaxAliveMs) {
