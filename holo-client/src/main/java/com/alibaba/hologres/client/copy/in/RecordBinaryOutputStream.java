...
public class RecordBinaryOutputStream extends RecordOutputStream {
        ...
        for (Column column : record.getSchema().getColumnSchema()) {
            try {
                if (record.isSet(index)) {
                    fillByteBuffer(record.getObject(index), column);
                }
            } catch (Exception e) {
                ...
                if (conn == null) {
                    throw new IOException(
                            "unsupported type:"
                                    ...
                                    + "("
                                    + type
                                    + "). Please call RecordBinaryOutputSteam constructor with BaseConnection Param");
                }
                try {
                    // obj如果是List<>或Object[]，都尝试转成Array
                    Array array = ArrayUtil.objectToArray(conn, obj, column.getTypeName());
                    byte[] arrayBytes =
                            ArrayUtil.arrayToBinary(
                                    conn,
                                    array != null
    ...
    private static short[] encodeFromString(String num, short[] info) {
        ...
        while (digitCount-- > 0) {
            digits[d++] =
                    (short)
                            (((numDigs[i] * 10 + numDigs[i + 1]) * 10 + numDigs[i + 2]) * 10
                                    + numDigs[i + 3]);
            i += DEC_DIGITS;
        }
