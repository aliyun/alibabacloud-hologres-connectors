...
public class UpsertStatementBuilder {
    ...
    public UpsertStatementBuilder(HoloConfig config) {
        ...
        this.inputNumberAsEpochMsForDatetimeColumn =
                config.isInputNumberAsEpochMsForDatetimeColumn();
        this.inputStringAsEpochMsForDatetimeColumn =
                config.isInputStringAsEpochMsForDatetimeColumn();
    ...
    static class SqlCache<T> {
        ...
        public SqlTemplate computeIfAbsent(
    ...
    Map<Tuple4<TableSchema, TableName, CheckAndPutCondition, Expression>, SqlTemplate> deleteCache =
            new HashMap<>();
    boolean first = true;

    private SqlTemplate buildDeleteSqlTemplate(
            Tuple4<TableSchema, TableName, CheckAndPutCondition, Expression> tuple) {
        TableSchema schema = tuple.f0;
        TableName tableName = tuple.f1;
        CheckAndPutCondition checkAndPutCondition = tuple.f2;
        Expression expression = tuple.f3;
        first = true;
        ...
        } else if (expression != null && expression.conflictWhere != null) {
            sb.append(" and ").append(expression.conflictWhere);
        }
        sb.append(")");
        ...
        int maxLevel =
                32
                        - Integer.numberOfLeadingZeros(
                                Short.MAX_VALUE / schema.getKeyIndex().length)
                        - 1;
    ...
    /**
     * 对delete语句， check and put 方式为`delete from table where (pk = ? and checkColumn < ?) or (pk = ?
     * and checkColumn < ?);`
     */
    protected String buildCheckAndDeletePattern(CheckAndPutCondition checkAndPutCondition) {
        ...
        String oldColumnPattern =
                coalesceNullValue(
                        checkAndPutCondition,
                        IdentifierUtil.quoteIdentifier(
                                checkAndPutCondition.getCheckColumn().getName(), true));
    ...
    private SqlTemplate buildInsertSql(
            Tuple5<TableSchema, TableName, OnConflictAction, CheckAndPutCondition, Expression>
                    tuple,
            Tuple<BitSet, BitSet> input) {
        TableSchema schema = tuple.f0;
        TableName tableName = tuple.f1;
        OnConflictAction mode = tuple.f2;
        ...
        if (schema.getKeyIndex().length > 0) {
            ...
            sb.append(") do ");
            if (OnConflictAction.INSERT_OR_IGNORE == mode) {
                sb.append("nothing");
            } else if (expression != null && expression.hasConflictUpdateSet()) {
                sb.append("update set ").append(expression.conflictUpdateSet);
            } else {
            ...
            if (checkAndPutCondition != null) {
                ...
                sb.append(" where ").append(expression.conflictWhere);
            }
        }
    ...
    protected String buildCheckAndPutPattern(CheckAndPutCondition checkAndPutCondition) {
        ...
        String oldColumnPattern =
                coalesceNullValue(
                        checkAndPutCondition,
                        getCheckColumnNameAlias(checkAndPutCondition, /*old*/ true));
        String newColumnPattern =
                " " + getCheckColumnNameAlias(checkAndPutCondition, /*old*/ false);
    ...
    private static String getCheckColumnNameAlias(
            CheckAndPutCondition checkAndPutCondition, boolean old) {
        return (old ? "old." : "excluded.")
                + IdentifierUtil.quoteIdentifier(
                        checkAndPutCondition.getCheckColumn().getName(), true);
    }

    private static String coalesceNullValue(
            CheckAndPutCondition checkAndPutCondition, String name) {
        StringBuilder sb = new StringBuilder();
        if (checkAndPutCondition.getNullValue() != null) {
            sb.append("coalesce(")
                    .append(name)
                    .append(", '")
                    .append(checkAndPutCondition.getNullValue())
                    .append("'::")
                    .append(checkAndPutCondition.getCheckColumn().getTypeName())
                    .append(")");
        } else {
            sb.append(name);
        }
    ...
    /**
     * 給所有非serial, 非生成列的且没有set的列都set成null.
     *
     * @param record
     */
    ...
    protected void handleArrayColumn(Connection conn, Record record, Column column, int index)
            throws SQLException {
    ...
    public void prepareRecord(Connection conn, Record record, OnConflictAction mode)
            throws SQLException {
        try {
            for (int i = 0; i < record.getSize(); ++i) {
                Column column = record.getSchema().getColumn(i);
                if (column.isGeneratedColumn() && record.isSet(i)) {
                    throw new SQLException(
                            String.format("generated column %s can not be set", column.getName()),
                            PSQLState.INVALID_PARAMETER_VALUE.getState());
                }
                if (record.getType() == Put.MutationType.INSERT
                        && mode != OnConflictAction.INSERT_OR_UPDATE) {
            ...
            throw new SQLException(e.getMessage(), PSQLState.INVALID_PARAMETER_VALUE.getState(), e);
        }
    }
    ...
    private void fillPreparedStatement(PreparedStatement ps, int index, Object obj, Column column)
            throws SQLException {
        switch (column.getType()) {
            case Types.OTHER:
                if (obj instanceof byte[]
                        && "roaringbitmap".equalsIgnoreCase(column.getTypeName())) {
            ...
            case Types.TIMESTAMP:
                if (obj instanceof Number && inputNumberAsEpochMsForDatetimeColumn) {
                    ps.setObject(
                            index, new Timestamp(((Number) obj).longValue()), column.getType());
            ...
            case Types.DATE:
                if (obj instanceof Number && inputNumberAsEpochMsForDatetimeColumn) {
                    ps.setObject(
                            index, new java.sql.Date(((Number) obj).longValue()), column.getType());
                } else if (obj instanceof String && inputStringAsEpochMsForDatetimeColumn) {
                    long l = 0L;
                    try {
                        l = Long.parseLong((String) obj);
    ...
    private int fillPreparedStatementForInsert(PreparedStatement ps, int psIndex, Record record)
            throws SQLException {
    ...
    protected void buildInsertStatement(
        ...
        SqlTemplate sql =
                insertCache.computeIfAbsent(
                        new Tuple5<>(schema, tableName, mode, checkAndPutCondition, expression),
                        columnSet,
                        this::buildInsertSql);
        fillPreparedStatement(
                ...
                list,
                recordList,
                Put.MutationType.INSERT,
                this::fillPreparedStatementForInsert);
    }
    ...
    private void fillPreparedStatement(
            ...
            FillPreparedStatementFunc func)
            throws SQLException {
                    ...
                    if (ps == null) {
                        ps =
                                new PreparedStatementWithBatchInfo(
                                        conn.prepareStatement(sqlTemplate.getSql(currentLevel)),
                                        fullValueBlocksCount > 1,
                                        type);
                        list.add(ps);
                    }
    ...
    private int fillPreparedStatementForDelete(PreparedStatement ps, int psIndex, Record record)
            throws SQLException {
        ...
        if (record instanceof CheckAndPutRecord) {
            CheckAndPutCondition checkAndPutCondition =
                    ((CheckAndPutRecord) record).getCheckAndPutCondition();
            // 已有数据和常量做比较，不需要传入参数
            if (checkAndPutCondition.isNewValueCheckWithOldValue()) {
    ...
    protected void buildDeleteStatement(
            Connection conn,
            HoloVersion version,
            TableSchema schema,
            TableName tableName,
        ...
        SqlTemplate sql =
                deleteCache.computeIfAbsent(
                        new Tuple4<>(schema, tableName, checkAndPutCondition, expression),
                        this::buildDeleteSqlTemplate);
        fillPreparedStatement(
                conn,
                sql,
                list,
                recordList,
                Put.MutationType.DELETE,
                this::fillPreparedStatementForDelete);
    }
    ...
    class SqlTemplate {
        ...
        public SqlTemplate(
                String header, String tail, String rowText, String delimiter, int maxLevel) {
            this.header = header;
            this.tail = tail;
        ...
        public String getSql(int level) {
            if (level >= sqls.length) {
                throw new RuntimeException(
                        this + " max level is " + sqls.length + ", but input level is " + level);
            }
            if (sqls[level] == null) {
                StringBuilder sb = new StringBuilder();
                if (header != null) {
     ...
     * @throws SQLException
     */
    public List<PreparedStatementWithBatchInfo> buildStatements(
            Connection conn,
            HoloVersion version,
            TableSchema schema,
            TableName tableName,
            Collection<Record> recordList,
            OnConflictAction mode)
            throws SQLException {
        ...
        try {
                    ...
                    case INSERT:
                        insertRecordList
                                .computeIfAbsent(
                                        new Tuple<>(
                                                record.getBitSet(),
                                                record.getOnlyInsertColumnSet()),
                                        t -> new ArrayList<>())
                                .add(record);
                        break;
                    default:
                        throw new SQLException(
                                "unsupported type:" + record.getType() + " for record:" + record);
                }
            }
            ...
            try {
                ...
                for (Map.Entry<Tuple<BitSet, BitSet>, List<Record>> entry :
                        insertRecordList.entrySet()) {
