            ...
            for (PrimitiveIterator.OfInt it = columnMask.stream().iterator(); it.hasNext(); ) {
                ...
                sb.append(
                        IdentifierUtil.quoteIdentifier(
                                schema.getColumn(it.next()).getName(), true));
            }
            ...
            if (scan.getFilterList() != null) {
                ...
                for (Filter filter : scan.getFilterList()) {
                    ...
                    if (filter instanceof EqualsFilter) {
                        sb.append(
                                        IdentifierUtil.quoteIdentifier(
                                                schema.getColumn(((EqualsFilter) filter).getIndex())
                                                        .getName(),
                                                true))
                                .append("=?");
            ...
            switch (scan.getSortKeys()) {
                ...
                case CLUSTERING_KEY:
                    if (schema.getClusteringKey() != null && schema.getClusteringKey().length > 0) {
                        sortKeyNames =
                                Arrays.stream(schema.getClusteringKey())
                                        ...
                                        if (filter instanceof EqualsFilter) {
                                            ...
                                            if (rf.getStart() != null) {
                                                ps.setObject(
                                                        ++paramIndex,
                                                        rf.getStart(),
                                                        schema.getColumn(rf.getIndex()).getType());
                                            }
                                            if (rf.getStop() != null) {
                                                ps.setObject(
                                                        ++paramIndex,
                                ...
                                try {
                                    ...
                                    scanAction
                                            ...
                                            .completeExceptionally(
                                                    HoloClientException.fromSqlException(e));
                                }
                                if (rs != null) {
                                    byte[] lock = new byte[] {};
                                    ...
                                    registry.histogram(Metrics.METRICS_SCAN_LATENCY)
