...
public class HologresJdbcConfigs {
	public static final String HOLOGRES_TABLE_TYPE = "hologres-jdbc";

	public static final String DEFAULT_FIELD_DELIMITER = "\u0002";

	public static final ConfigOption<Integer> OPTIONAL_WRITE_BATCH_SIZE = ConfigOptions.key("batchSize".toLowerCase()).intType().defaultValue(-1);
	...
	public static final ConfigOption<Boolean> INSERT_OR_UPDATE = key("insertOrUpdate".toLowerCase()).booleanType().defaultValue(false);

	public static final ConfigOption<Boolean> CREATE_MISSING_PARTITION_TABLE = key("createPartTable".toLowerCase()).booleanType().defaultValue(false);

	public static final ConfigOption<String> MUTATE_TYPE = key("mutateType".toLowerCase()).stringType().defaultValue("insertOrIgnore");
	...
	public static final ConfigOption<Long> OPTIONAL_SPLIT_DATA_SIZE = ConfigOptions.key("splitDataSize".toLowerCase()).longType().defaultValue(10 * 1024 * 1024L);
	public static final ConfigOption<Integer> OPTIONAL_SPLIT_LENGTH = ConfigOptions.key("splitLength".toLowerCase()).intType().defaultValue(512);
	...
	public static Set<ConfigOption<?>> getAllOption() {
		...
		HologresJdbcConfigs config = new HologresJdbcConfigs();
		Arrays.stream(FieldUtils.getAllFields(HologresJdbcConfigs.class))
				.filter(f -> ConfigOption.class.isAssignableFrom(f.getType())).forEach(f -> {
			try {
