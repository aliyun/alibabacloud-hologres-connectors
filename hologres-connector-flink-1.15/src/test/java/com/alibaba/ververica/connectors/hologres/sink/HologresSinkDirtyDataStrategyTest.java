...
public class HologresSinkDirtyDataStrategyTest extends HologresTestBase {
    ...
    @Parameterized.Parameters(name = "use writeMode = {0}")
    public static List<String> parameters() {
        return Arrays.asList("EXCEPTION", "SKIP_SILENT", "SKIP");
    }

    public HologresSinkDirtyDataStrategyTest(String dirtyDataStrategy) throws IOException {
        this.dirtyDataStrategy = DirtyDataStrategy.valueOf(dirtyDataStrategy);
    }
    ...
    public void testSinkTableNoDirtyData() throws Exception {
        tEnv.executeSql(
                "create table sinkTable"
                        ...
                        + "'remove-u0000-in-text.enabled'='true',\n"
                        + "'sink.error-handler'='"
    ...
    public void testSinkTableDirtyDataU0000() throws Exception {
        tEnv.executeSql(
                "create table sinkTable"
                        ...
                        + "'connector'='hologres',\n"
                        + "'mutateType'='insertorupdate',"
                        + "'remove-u0000-in-text.enabled'='false',\n"
                        + "'sink.error-handler'='"
                        + dirtyDataStrategy.name()
                        + "',\n"
                        + "'endpoint'='"
